...
function diagnosticFieldsPresent(artifact) {
  return DIAGNOSTIC_SOURCE_FIELDS.every((field) => typeof artifact?.[field] === "boolean");
}

function buildDataAdequacy(sourceRuntime, ready) {
...
function buildComparisonDesignAdequacy(sourceRuntime, ready, evidenceReady) {
  return {
    aggregate_measurement_cell_grain:
...
function buildModelDiagnostics(sourceRuntime, ready, evidenceReady) {
...
function buildBoundaryChecks(sourceRuntime, ready) {
  const artifact = asRecord(sourceRuntime?.internal_fit_artifact);
  return {
    runtime_fixture_prototype_only:
...
function reviewedFixtureArtifactRef(sourceRuntime, ready) {
  const artifact = asRecord(sourceRuntime?.internal_fit_artifact);
  return {
...
function buildPromotionReview(ready, evidenceReady = false) {
  return {
...
export function contributionAlignmentInternalDiagnosticsModelAdequacyReviewHash(review) {
  const withoutHash = clone(review);
  delete withoutHash.review_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentInternalDiagnosticsModelAdequacyReviewFromObject(input) {
...
export function validateContributionAlignmentInternalDiagnosticsModelAdequacyReview(review, options = {}) {
