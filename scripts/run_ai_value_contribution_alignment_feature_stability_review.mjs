...
function sourceDataModelFromInput(input) {
...
function sourceRef(sourceDataModel) {
  return {
    schema_version:
  ...
  if (value && typeof value === "object") {
    ...
    for (const [key, nested] of Object.entries(value)) {
      ...
      const allowedBlockedUse =
        path === "review.blocked_uses" && REQUIRED_BLOCKED_USES.includes(nested);
      const allowedCaveat =
        path === "review.required_caveats" && REQUIRED_CAVEATS.includes(nested);
      if (
        !safeFalseFlag &&
        !allowedBlockedUse &&
        !allowedCaveat &&
...
function sourceDataModelGaps(sourceDataModel) {
  ...
  if (source.data_model_hash !== contributionAlignmentInternalResearchMathDataModelHash(source)) {
...
function buildFeatureRegistry(sourceDataModel, ready) {
  if (!ready) return [];
...
function baseReview(sourceDataModel, state, gaps) {
...
export function contributionAlignmentFeatureStabilityReviewHash(review) {
  const withoutHash = clone(review);
  delete withoutHash.review_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentFeatureStabilityReviewFromObject(input) {
...
export function validateContributionAlignmentFeatureStabilityReview(review, options = {}) {
...
function main() {
  ...
  if (!inputPath) {
    console.error(
      "Usage: node scripts/run_ai_value_contribution_alignment_feature_stability_review.mjs <source-data-model-json|- for stdin>"
    );
    process.exit(1);
  }
  const review = buildContributionAlignmentFeatureStabilityReviewFromObject(inputFromCliPath(inputPath));
