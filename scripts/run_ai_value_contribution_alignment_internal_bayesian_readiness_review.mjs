...
function sourceFrameFromInput(input) {
  const record = asRecord(input);
  return record.source_frame ?? input;
}

function sourceWeightObjectFromInput(input, options = {}) {
  const record = asRecord(input);
...
function sourceWeightObjectRef(sourceWeightObject, sourceFrame) {
...
function sourceFrameGaps(sourceFrame) {
...
function sourceWeightObjectGaps(sourceWeightObject, sourceFrame) {
...
function buildReviewedFeatureWeights(sourceFrame) {
...
function buildReview(sourceFrame, sourceWeightObject, state, gaps) {
...
export function contributionAlignmentInternalBayesianReadinessReviewHash(review) {
  const withoutHash = clone(review);
  delete withoutHash.review_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentInternalBayesianReadinessReviewFromObject(input, options = {}) {
...
export function validateContributionAlignmentInternalBayesianReadinessReview(review, options = {}) {
