...
function sourceWeightObjectFromInput(input) {
  const record = asRecord(input);
  return record.source_weight_object ?? input;
}

function sourceWeightDecisionFromInput(input, options = {}) {
  const record = asRecord(input);
  return options.sourceWeightDecision ?? record.source_weight_decision ?? null;
}

function sourceFeatureReviewFromInput(input, options = {}) {
  const record = asRecord(input);
...
function sourceWeightObjectRef(sourceWeightObject) {
  return {
    schema_version:
...
function sourceWeightDecisionRef(sourceDecision, sourceWeightObject) {
...
function sourceFeatureReviewRef(sourceReview, sourceWeightObject) {
...
function hasForbiddenContent(value, path = "frame") {
...
function sourceFeatureReviewGaps(sourceReview, sourceWeightObject) {
...
function sourceWeightDecisionGaps(sourceDecision, sourceWeightObject) {
...
function sourceWeightObjectGaps(sourceWeightObject) {
...
function buildComposition(sourceWeightObject) {
...
function buildFrame(sourceWeightObject, sourceDecision, sourceReview, state, gaps) {
...
export function contributionAlignmentWeightedInternalModelFrameHash(frame) {
  const withoutHash = clone(frame);
  delete withoutHash.frame_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentWeightedInternalModelFrameFromObject(input, options = {}) {
...
function rejectValidation(frame) {
...
function collectShapeGaps(frame) {
...
function collectSourceBindingGaps(frame, options = {}) {
  const gaps = [];
...
export function validateContributionAlignmentWeightedInternalModelFrame(frame, options = {}) {
  if (frame?.frame_state === REJECT_STATE) {
    return rejectValidation(frame);
  }
  const gaps = sanitizeGaps([
...
function main() {
  ...
  if (!inputPath) {
    console.error(
      "Usage: node scripts/run_ai_value_contribution_alignment_weighted_internal_model_frame.mjs <versioned-weight-object-json|- for stdin>"
    );
    process.exit(1);
  }
  const frame = buildContributionAlignmentWeightedInternalModelFrameFromObject(inputFromCliPath(inputPath));
