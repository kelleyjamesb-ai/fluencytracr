...
function promotionGateRef(gate, accepted) {
  return {
...
function promotionAuthority(sources) {
...
function blockedOutputProof(sources) {
...
function buildHandoff(sources, state, gaps) {
...
export function contributionAlignmentPromotionGatePassedArtifactHandoffHash(handoff) {
  const withoutHash = clone(handoff);
  delete withoutHash.handoff_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentPromotionGatePassedArtifactHandoffFromObject(input) {
...
export function validateContributionAlignmentPromotionGatePassedArtifactHandoff(handoff, options = {}) {
