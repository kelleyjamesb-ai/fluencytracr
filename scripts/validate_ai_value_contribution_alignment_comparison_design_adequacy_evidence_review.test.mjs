...
test("comparison-design adequacy review holds plain runtime without source envelope for reviewer-owned packages", () => {
  const runtime = sourceRuntime();
  const { collection } = reviewerOwnedCollection();
  const review =
    buildContributionAlignmentComparisonDesignAdequacyEvidenceReviewFromObject({
  ...
  assert.ok(
    review.validation_summary.gaps.some((gap) => /source_runtime\.sourceGate is required/.test(gap)),
    review.validation_summary.gaps.join("; ")
  );
});
