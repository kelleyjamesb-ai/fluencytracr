...
function sourceDiagnosticsReviewFromInput(input) {
...
function inputBoundaryGaps(input) {
  const record = asRecord(input);
  if (!Object.prototype.hasOwnProperty.call(record, "source_diagnostics_review")) return [];
...
function sourceRuntimeRef(sourceRuntime, review) {
...
function sourceDiagnosticsReviewRef(review) {
  return {
    schema_version:
...
function sourceDiagnosticsEvidencePacketRef(packet) {
  return {
    schema_version:
...
function sourceFixtureArtifactRef(review, sourceRuntime) {
...
function sourceDiagnosticsEvidencePacketGaps(packet, review, sourceRuntime) {
  const gaps = [];
...
function sourceRuntimeGaps(sourceRuntime, review) {
  ...
  if (validation.valid !== true) {
    ...
    gaps.push(...validation.gaps.map((gap) => `sourceRuntime.${gap}`));
  }
...
function sourceReviewLeakageGaps(review) {
  const gaps = [];
...
function hasForbiddenSourceDiagnosticsReviewContent(value, path = "sourceDiagnosticsReview") {
...
function buildSourceStateChecks(review, sourceRuntime, sourceDiagnosticsEvidencePacket, sourceGaps) {
  const runtime = asRecord(sourceRuntime);
  const artifact = asRecord(runtime.internal_fit_artifact);
  const reviewArtifactRef = asRecord(review?.reviewed_fixture_artifact_ref);
  return {
...
function buildDiagnosticsSufficiency(review, sourceRuntime, sourceDiagnosticsEvidencePacket) {
  const data = asRecord(review?.data_adequacy);
  const diagnostics = asRecord(review?.model_diagnostics);
  const comparison = asRecord(review?.comparison_design_adequacy);
...
function buildGovernanceContainment(review, sourceRuntime) {
  const boundary = asRecord(review?.boundary_checks);
  const runtime = asRecord(sourceRuntime);
  return {
    runtime_fixture_prototype_only:
...
function buildFeatureWeightPolicy(review) {
  const boundary = asRecord(review?.boundary_checks);
  return {
    weights_structural_internal_only:
...
function buildPromotionDecision(passed) {
  return {
...
function buildGate(review, sourceRuntime, sourceDiagnosticsEvidencePacket, state, sourceGaps) {
...
export function contributionAlignmentBayesianPromotionDecisionGateHash(gate) {
  const withoutHash = clone(gate);
  delete withoutHash.gate_hash;
  return sha256Json(withoutHash);
}

function sourceReviewRejectedForBoundary(review) {
  const policy = asRecord(review?.review_policy);
  for (const field of [
    "posterior_interpretation_authorized",
    "posterior_output_authorized",
...
export function buildContributionAlignmentBayesianPromotionDecisionGateFromObject(input) {
...
export function validateContributionAlignmentBayesianPromotionDecisionGate(gate, options = {}) {
