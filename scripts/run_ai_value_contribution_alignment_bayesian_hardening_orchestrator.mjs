...
function stateOf(step, artifact) {
...
function promotionAuthorizedOf(step, artifact) {
...
function allowedNextStepForBlocked(steps, firstBlocked) {
  if (!firstBlocked) return steps.at(-1)?.allowed_next_step ?? null;
...
function explicitArtifacts(input) {
...
function explicitPathSummary(input, sourceRuntimeSource) {
  ...
  return {
    ...
    first_blocked_gate: firstBlocked?.step ?? null,
    current_state: firstBlocked?.state ?? steps.at(-1)?.state ?? null,
    allowed_next_step: allowedNextStepForBlocked(steps, firstBlocked),
    steps
  };
}

function artifactHashes(sourceRuntime, defaultExecution, explicitPath) {
  const explicitSteps = Object.fromEntries(
    explicitPath.steps.map((step) => [step.step, step])
  );
  const defaultSteps = Object.fromEntries(
    defaultExecution.steps.map((step) => [step.step, step])
  );
  const step = (name) => explicitSteps[name]?.hash ?? defaultSteps[name]?.hash ?? null;
  return {
...
function promotionAuthority(explicitPath) {
  const gateStep = explicitPath.steps.find((step) => step.step === "bayesian_promotion_decision_gate");
  const nonGatePromotion = explicitPath.steps.some((step) =>
...
export function contributionAlignmentBayesianHardeningOrchestratorReportHash(report) {
  const withoutHash = clone(report);
  delete withoutHash.report_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentBayesianHardeningOrchestratorReportFromObject(input) {
...
export function validateContributionAlignmentBayesianHardeningOrchestratorReport(
  ...
  if (record.verification_status?.explicit_path_validated !== true) {
    const invalidSuppliedSteps = asRecord(record.explicit_governed_path).steps?.filter?.((step) =>
