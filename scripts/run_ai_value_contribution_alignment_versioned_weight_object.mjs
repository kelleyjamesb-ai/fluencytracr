...
function sourceWeightDecisionFromInput(input) {
...
function sourceWeightDecisionRef(sourceDecision) {
  return {
    schema_version:
...
function sourceFeatureReviewRef(sourceReview, sourceDecision) {
...
function hasForbiddenContent(value, path = "weight_object") {
...
function sourceFeatureReviewGaps(sourceReview, sourceDecision) {
...
function sourceWeightDecisionGaps(sourceDecision) {
...
function buildWeightObject(sourceDecision, sourceReview, state, gaps) {
...
export function contributionAlignmentVersionedWeightObjectHash(weightObject) {
  const withoutHash = clone(weightObject);
  delete withoutHash.weight_object_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentVersionedWeightObjectFromObject(input, options = {}) {
...
function rejectValidation(weightObject) {
...
function collectShapeGaps(weightObject) {
...
function collectSourceBindingGaps(weightObject, options = {}) {
  const gaps = [];
...
export function validateContributionAlignmentVersionedWeightObject(weightObject, options = {}) {
  if (weightObject?.weight_object_state === REJECT_STATE) {
    return rejectValidation(weightObject);
  }
  const gaps = sanitizeGaps([
