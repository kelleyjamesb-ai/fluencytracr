...
function collectObjectiveGaps(engagement: any): string[] {
  ...
  if (!objective) {
    gaps.push("business_objective is missing");
    return gaps;
  }
  for (const field of [
    "objective_id",
    ...
    requireField(objective, field, "business_objective", gaps);
  }
  return gaps;
}
...
export function validateEngagement(engagement: any): EngagementValidationResult {
  ...
  return {
    ...
    objective_id: engagement?.business_objective?.objective_id ?? null,
