/**
 * AI Value Engine — Workflow Blueprint stage.
 *
 * Owns Blueprint validation and the deterministic adapters that feed the
 * workshop response and support value input. Logic migrated verbatim from
 * scripts/validate_ai_value_blueprint.mjs per the migration contract in
 * docs/concepts/AI_VALUE_ENGINE_CONTRACT.md.
 */

const RESULT_SCHEMA_VERSION = "FT_AI_VALUE_BLUEPRINT_VALIDATION_2026_06";

export const BLUEPRINT_ALLOWED_VALUE_ROUTES = new Set([
  "COST_REDUCTION",
  "CAPACITY_CREATION",
  "QUALITY_IMPROVEMENT",
  "RISK_REDUCTION",
...
export interface BlueprintValidationResult {
  ...
  gaps: string[];
  feeds: {
    workshop_response: boolean;
    support_value_input: boolean;
  };
}
...
function collectRequiredFieldGaps(blueprint: any): string[] {
  const gaps: string[] = [];
  for (const field of [
    "schema_version",
    "blueprint_id",
...
function collectProcessGaps(blueprint: any): string[] {
  const gaps: string[] = [];
...
function collectSourceRequirementGaps(blueprint: any): string[] {
  const gaps: string[] = [];
...
function collectAssumptionGaps(blueprint: any): string[] {
  const gaps: string[] = [];
  const assumptionById = new Map(
    (blueprint?.assumption_ledger ?? []).map((assumption: any) => [
  ...
  for (const assumptionId of REQUIRED_ASSUMPTIONS) {
    const assumption: any = assumptionById.get(assumptionId);
    if (!assumption) {
...
function collectBlockedClaimGaps(blueprint: any): string[] {
  const gaps: string[] = [];
...
function collectGovernanceGaps(blueprint: any): string[] {
  const gaps: string[] = [];
...
export function blueprintToWorkshopResponse(blueprint: any): any {
  return {
...
export function blueprintToSupportValueInput(blueprint: any): any {
  const aggregateInputs = blueprint.source_requirements.approved_aggregate_inputs;
...
export function validateBlueprint(blueprint: any): BlueprintValidationResult {
  const gaps = [
