/**
 * AI Value Engine — Value Scenario stage.
 *
 * Owns governed value scenario validation and the deterministic draft
 * builder from validated Blueprint + Metrics Library objects. Logic migrated
 * verbatim from scripts/validate_ai_value_scenario.mjs per the migration
 * contract.
 */
...
function collectInputGaps(scenario: any): string[] {
  const gaps: string[] = [];
  ...
  if (!Array.isArray(input.metric_references) ||
    ...
    input.metric_references.forEach((metric: any, index: number) => {
  ...
  if (!Array.isArray(input.customer_owned_assumptions) ||
    ...
    input.customer_owned_assumptions.forEach((assumption: any, index: number) => {
      requireField(
  ...
  if (!Array.isArray(input.scenario_bands) ||
    ...
    const bands = new Set(input.scenario_bands.map((band: any) => band.band));
    for (const band of REQUIRED_SCENARIO_BANDS) {
      if (!bands.has(band)) {
        gaps.push(`input.scenario_bands missing ${band}`);
      }
    }
    input.scenario_bands.forEach((band: any, index: number) => {
...
function collectOutputGaps(scenario: any): string[] {
  const gaps: string[] = [];
...
function collectBlockedClaimGaps(scenario: any): string[] {
  const gaps: string[] = [];
...
function metricReferencesForPrimaryRoute(recommendation: any, primaryRoute: string): any[] {
  return recommendation.recommended_metrics
    .filter((metric: any) => metric.value_route === primaryRoute)
    .map((metric: any) => ({
...
function buildScenarioBands(metricReferences: any[]): any[] {
...
export function validateValueScenario(scenario: any): ScenarioValidationResult {
  const gaps = [
