/**
 * AI Value Engine — Metrics Library stage.
 *
 * Owns Metrics Library validation and blueprint-driven metric
 * recommendation. Logic migrated verbatim from
...
export interface MetricsLibraryValidationResult {
  ...
  gaps: string[];
  feeds: {
    blueprint_metrics_mapping: boolean;
  };
}
...
function collectTopLevelGaps(library: any): string[] {
  const gaps: string[] = [];
  for (const field of [
    "schema_version",
    "library_id",
...
function collectMetricFieldGaps(metric: any, index: number): string[] {
  const gaps: string[] = [];
...
function uniqueRoutesFromBlueprint(blueprint: any): any[] {
  return [
...
export function validateMetricsLibrary(library: any): MetricsLibraryValidationResult {
  const metricGaps = Array.isArray(library?.metrics)
    ? library.metrics.flatMap((metric: any, index: number) =>
        collectMetricFieldGaps(metric, index)
      )
    : [];
  const gaps = [
...
export function recommendMetricsForBlueprint(blueprint: any, library: any): any {
  ...
  return {
    ...
    recommended_metrics: recommendedMetrics.map((metric: any) => ({
