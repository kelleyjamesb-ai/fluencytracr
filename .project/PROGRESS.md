...
- AI Value Frontend Accessibility/UX closeout slice (2026-07-03): closed the remaining frontend-only form, live-region, step-indicator, and empty-state gaps across Discovery and the AI Value workspace. Required Discovery and Evidence Checkpoint intake fields now expose `aria-required`, `aria-invalid`, and error descriptions; async/held regions announce politely; the workspace header shows Step 1-7 while keeping `aria-current` on the active nav step only; and empty states name the missing input, why it matters for planning, and the unlocking action. This added no backend, scripts, schemas, shared, OpenSpec, new routes, dependencies, design-token color changes, ROI proof, causality, productivity, confidence math, probability, or customer-facing financial output. Verification: `npm run build --workspace shared` prerequisite for frontend tests; `npm test --workspace frontend` (15 files/136 tests); `npm run build --workspace frontend`; Playwright keyboard-only walkthrough of `/ai-value-workspace` through all 7 steps; `git diff --check` passed.
