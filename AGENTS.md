...
- Depth Repertoire remains `HOLD_FOR_FIXED_WINDOW_VALIDATION` until docs/research/V4_DEPTH_REPERTOIRE_STABILITY_READOUT.md is updated with fixed-window evidence that promotes the candidate. No V4 economic readout, Time-Saved Defensibility Range, or value-confidence API may depend on Depth Repertoire until promoted. Glean dogfood values must not be embedded into universal V4 concepts, contracts, schemas, thresholds, or defaults. If Depth Repertoire is later promoted, it must still be aggregate-only and must not rank individuals, teams, departments, customers, or managers.
