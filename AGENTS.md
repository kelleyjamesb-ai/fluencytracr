...
AI Work Evidence is the org-agnostic core layer for commercializing FluencyTracr beyond any single source system (see docs/concepts/AI_WORK_EVIDENCE.md). It separates reusable aggregate evidence primitives from source-specific adapters such as the current Glean dogfood and value-evidence work. Glean-specific names, dogfood values, claim IDs, and adapter assumptions must not become universal core contracts.
...
AI Manager Outcomes Recommendations is a V4 docs-first concept (see docs/concepts/AI_MANAGER_OUTCOMES_RECOMMENDATIONS.md). It uses aggregate AI Work Evidence to recommend customer-owned outcome signals and aggregate testing formulas for cost, revenue, quality, capacity, risk, or experience value routes. "AI Manager" means the AI program owner or value-realization leader, not a people manager. It does not require HRIS, survey, enablement, training, finance, or person-level data at pilot start; it does not calculate ROI, prove causality, measure productivity, score employees, rank teams/managers/departments, or create customer-facing economic output.
...
- AI Work Evidence: org-agnostic core layer for aggregate surfaces, workflows, approved cohorts, interventions, trust evidence, source coverage, outcome evidence, and value hypotheses; source adapters map into it
...
- AI Manager Outcomes Recommendations: V4 docs-first layer that recommends customer-owned outcome signals and aggregate formula templates for testing cost, revenue, quality, capacity, risk, or experience value routes; it is not ROI, causality, productivity, HR analytics, workforce assessment, or comparative group evaluation
...
- docs/research/V4_CANONICAL_CONTRACT_DECISION.md records `PROMOTE_INTERNAL_READOUT_CONTRACT_HARDENING` and `HOLD_CUSTOMER_FACING_ECONOMIC_OUTPUT`. The only promoted next scope is documentation-first internal readout contract hardening. No V4 economic API, Time-Saved Defensibility Range output, ROI, productivity claim, causal claim, prediction claim, ranking surface, schema, endpoint, runtime service, or frontend product surface may start unless a later decision promotes that exact scope.
- docs/contracts/value-confidence/internal-scale-readiness-readout.md is the docs-only internal readout contract created from that decision. It defines aggregate record shape, caveats, blocked fields, and example surfaced/held records only. It does not authorize APIs, schemas, runtime services, frontend surfaces, customer-facing economic output, Time-Saved range values, ROI, productivity, causality, prediction, Trust Calibration promotion, reusable leverage promotion, or Skill Read Evidence promotion.
- docs/contracts/value-confidence/ai-manager-outcomes-recommendations.md is the docs-only recommendation contract for AI Manager Outcomes Recommendations. It may be used for internal readout language, pilot packet examples, and review checklists only. It does not authorize APIs, schemas, runtime services, automated recommendation engines, formula execution, outcome joins, customer-facing economic output, ROI, productivity, causality, prediction, HR analytics, or ranking surfaces.
