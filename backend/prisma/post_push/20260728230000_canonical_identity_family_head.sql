...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'canonical_identity_family_source_kind_check'
      AND conrelid =
    ...
    ALTER TABLE public.ai_value_canonical_identity_family_head_journal
      ADD CONSTRAINT "canonical_identity_family_source_kind_check"
      CHECK (
        source_kind IN (
          'VALUE_HYPOTHESIS',
          'MEASUREMENT_PLAN',
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_constraint
    WHERE conname = 'canonical_identity_family_identity_check'
      AND conrelid =
        'public.ai_value_canonical_identity_family_head_journal'::regclass
  ) THEN
    ALTER TABLE public.ai_value_canonical_identity_family_head_journal
      ADD CONSTRAINT "canonical_identity_family_identity_check"
      CHECK (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_constraint
    WHERE conname = 'canonical_identity_family_root_check'
      AND conrelid =
        'public.ai_value_canonical_identity_family_head_journal'::regclass
  ) THEN
    ALTER TABLE public.ai_value_canonical_identity_family_head_journal
      ADD CONSTRAINT "canonical_identity_family_root_check"
      CHECK (
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_constraint
    WHERE conname = 'canonical_identity_family_attestation_check'
      AND conrelid =
        'public.ai_value_canonical_identity_family_head_journal'::regclass
  ) THEN
    ALTER TABLE public.ai_value_canonical_identity_family_head_journal
      ADD CONSTRAINT "canonical_identity_family_attestation_check"
      CHECK (
        (
...
CREATE INDEX IF NOT EXISTS "canonical_identity_family_tail_idx"
ON public.ai_value_canonical_identity_family_head_journal(
  source_kind,
  org_id,
  stable_source_id,
...
BEGIN
  IF EXISTS (
    ...
    LEFT JOIN public.ai_value_canonical_identity_family_head_journal AS journal
      ON journal.source_kind = source_row.source_kind
     AND journal.org_id = source_row.org_id
     AND journal.stable_source_id = source_row.stable_source_id
     AND journal.version = source_row.version
     AND journal.source_row_id = source_row.source_row_id
     AND journal.predecessor_row_id IS NOT DISTINCT FROM
       source_row.predecessor_row_id
    WHERE journal.source_row_id IS NULL
  ) THEN
    RAISE EXCEPTION 'canonical identity post-push journal readback failed'
      USING ERRCODE = 'integrity_constraint_violation';
  END IF;
END
$readback$;

DROP TRIGGER IF EXISTS "value_hypotheses_canonical_identity_append"
  ON public.value_hypotheses;
DROP TRIGGER IF EXISTS "measurement_plans_canonical_identity_append"
  ON public.measurement_plans;
DROP TRIGGER IF EXISTS "measurement_cell_snapshots_canonical_identity_append"
  ON public.measurement_cell_snapshots;
DROP TRIGGER IF EXISTS "value_hypotheses_canonical_identity_append_only"
  ON public.value_hypotheses;
DROP TRIGGER IF EXISTS "measurement_plans_canonical_identity_append_only"
  ON public.measurement_plans;
DROP TRIGGER IF EXISTS
  "measurement_cell_snapshots_canonical_identity_append_only"
  ON public.measurement_cell_snapshots;
DROP TRIGGER IF EXISTS "canonical_identity_family_head_journal_append_only"
  ON public.ai_value_canonical_identity_family_head_journal;
...
DROP POLICY IF EXISTS "value_hypotheses_slice_e_runtime_select"
  ON public.value_hypotheses;
DROP POLICY IF EXISTS "value_hypotheses_slice_e_runtime_insert"
  ON public.value_hypotheses;
DROP POLICY IF EXISTS "measurement_plans_slice_e_runtime_select"
  ON public.measurement_plans;
DROP POLICY IF EXISTS "measurement_plans_slice_e_runtime_insert"
  ON public.measurement_plans;
DROP POLICY IF EXISTS "measurement_cell_snapshots_slice_e_runtime_select"
  ON public.measurement_cell_snapshots;
DROP POLICY IF EXISTS "measurement_cell_snapshots_slice_e_runtime_insert"
  ON public.measurement_cell_snapshots;
DROP POLICY IF EXISTS "canonical_identity_family_head_slice_e_runtime_select"
  ON public.ai_value_canonical_identity_family_head_journal;
