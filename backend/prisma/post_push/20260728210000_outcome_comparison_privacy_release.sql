...
ALTER TABLE "outcome_comparison_attestation_keys"
  ADD CONSTRAINT "outcome_comparison_attestation_key_shape_check"
    CHECK (
...
ALTER TABLE "outcome_comparison_attestation_key_activations"
  ADD CONSTRAINT "outcome_comparison_attestation_activation_key_fkey"
    FOREIGN KEY ("key_id")
    REFERENCES "outcome_comparison_attestation_keys"("key_id")
    ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE "outcome_comparison_attestation_key_revocations"
  ADD CONSTRAINT "outcome_comparison_attestation_revocation_reason_check"
    CHECK ("reason_code" ~ '^[A-Z][A-Z0-9_]{0,63}$'),
  ADD CONSTRAINT "outcome_comparison_attestation_revocation_key_fkey"
    FOREIGN KEY ("key_id")
    REFERENCES "outcome_comparison_attestation_keys"("key_id")
    ON DELETE RESTRICT ON UPDATE RESTRICT;

ALTER TABLE "outcome_comparison_privacy_releases"
  ADD CONSTRAINT "outcome_comparison_release_policy_check"
    CHECK ("policy_version" = 'FT_OUTCOME_COMPARISON_PRIVACY_POLICY_2026_07'),
  ADD CONSTRAINT "outcome_comparison_release_decision_check"
    CHECK ("decision" = 'ATOMIC_COMPARISON_PRIVACY_RELEASED'),
  ADD CONSTRAINT "outcome_comparison_release_identity_check"
    CHECK (
  ...
  ADD CONSTRAINT "outcome_comparison_release_evidence_ids_check"
    CHECK (
  ...
  ADD CONSTRAINT "outcome_comparison_release_descriptors_check"
    CHECK (
  ...
  ADD CONSTRAINT "outcome_comparison_release_hashes_check"
    CHECK (
  ...
  ADD CONSTRAINT "outcome_comparison_release_windows_check"
    CHECK (
  ...
  ADD CONSTRAINT "outcome_comparison_release_cohort_sizes_check"
    CHECK (
      "baseline_cohort_size" >= 5
      AND "comparison_cohort_size" >= 5
    ),
  ADD CONSTRAINT "outcome_comparison_release_values_check"
    CHECK (
  ...
  ADD CONSTRAINT "outcome_comparison_release_non_authority_check"
    CHECK (
  ...
  ADD CONSTRAINT "outcome_comparison_release_attestation_shape_check"
    CHECK (
...
ON public.outcome_comparison_attestation_keys FOR SELECT
TO fluencytracr_c1_attestation_provisioner USING (true);
CREATE POLICY "outcome_comparison_attestation_keys_provisioner_insert"
ON public.outcome_comparison_attestation_keys FOR INSERT
TO fluencytracr_c1_attestation_provisioner WITH CHECK (true);
CREATE POLICY "outcome_comparison_attestation_activations_provisioner"
ON public.outcome_comparison_attestation_key_activations FOR SELECT
TO fluencytracr_c1_attestation_provisioner USING (true);
CREATE POLICY "outcome_comparison_attestation_activations_provisioner_insert"
ON public.outcome_comparison_attestation_key_activations FOR INSERT
TO fluencytracr_c1_attestation_provisioner WITH CHECK (true);
CREATE POLICY "outcome_comparison_attestation_revocations_provisioner"
ON public.outcome_comparison_attestation_key_revocations FOR SELECT
TO fluencytracr_c1_attestation_provisioner USING (true);
CREATE POLICY "outcome_comparison_attestation_revocations_provisioner_insert"
ON public.outcome_comparison_attestation_key_revocations FOR INSERT
TO fluencytracr_c1_attestation_provisioner WITH CHECK (true);
...
ON public.outcome_comparison_privacy_releases FOR SELECT
TO fluencytracr_c1_runtime USING (true);
CREATE POLICY "outcome_comparison_privacy_releases_runtime_insert"
ON public.outcome_comparison_privacy_releases FOR INSERT
TO fluencytracr_c1_runtime WITH CHECK (true);
...
ON public.cohort_producer_authorities FOR SELECT
TO fluencytracr_c1_runtime USING (true);
CREATE POLICY "cohort_producer_authorities_c1_runtime_lock"
ON public.cohort_producer_authorities FOR UPDATE
TO fluencytracr_c1_runtime USING (true) WITH CHECK (false);
CREATE POLICY "cohort_producer_authority_revocations_c1_runtime"
ON public.cohort_producer_authority_revocations FOR SELECT
TO fluencytracr_c1_runtime USING (true);
CREATE POLICY "aggregate_privacy_reservations_c1_runtime"
ON public.aggregate_privacy_reservations FOR SELECT
TO fluencytracr_c1_runtime USING (true);
CREATE POLICY "cohort_proof_journal_c1_runtime"
ON public.cohort_proof_journal FOR SELECT
TO fluencytracr_c1_runtime USING (true);
...
ON public.ai_value_objects FOR SELECT
TO fluencytracr_c1_runtime USING (true);
CREATE POLICY "ai_value_objects_c1_runtime_lock"
ON public.ai_value_objects FOR UPDATE
TO fluencytracr_c1_runtime USING (true) WITH CHECK (false);
...
RETURNS BYTEA AS $$
  SELECT
    ...
    || public.outcome_comparison_attestation_frame(pg_catalog.convert_to(release_row.attestation_key_id, 'UTF8'))
    || public.outcome_comparison_attestation_frame(pg_catalog.uuid_send(release_row.id))
    || public.outcome_comparison_attestation_frame(
      pg_catalog.int8send((extract(epoch FROM release_row.created_at) * 1000)::BIGINT)
    )
    ...
    || public.outcome_comparison_attestation_frame(pg_catalog.int8send(
...
BEGIN
  IF session_user <> 'fluencytracr_c1_runtime' THEN
    RAISE EXCEPTION 'C.1 release insertion requires the direct runtime login'
      USING ERRCODE = 'insufficient_privilege';
  END IF;
  supplied_key_id := pg_catalog.current_setting('fluencytracr.c1_attestation_key_id', true);
  supplied_secret := pg_catalog.current_setting('fluencytracr.c1_attestation_secret', true);
  ...
  ORDER BY activation.activation_epoch DESC LIMIT 1;
...
CREATE OR REPLACE FUNCTION "verify_outcome_comparison_creation_attestation"(release_id UUID)
RETURNS BOOLEAN AS $$
DECLARE
  ...
  expected_hmac TEXT;
BEGIN
  IF session_user <> 'fluencytracr_c1_runtime' THEN RETURN false; END IF;
  supplied_key_id := pg_catalog.current_setting('fluencytracr.c1_attestation_key_id', true);
  supplied_secret := pg_catalog.current_setting('fluencytracr.c1_attestation_secret', true);
  PERFORM pg_catalog.pg_advisory_xact_lock(
    pg_catalog.hashtextextended('FT_C1_ATTESTATION_PROVISIONING_V1', 0)
  );
  SELECT * INTO release_row FROM public.outcome_comparison_privacy_releases WHERE id = release_id;
  IF NOT FOUND OR supplied_key_id IS NULL OR supplied_secret IS NULL
...
BEGIN
  ...
  IF configured_active_key_id IS NULL
     ...
     OR configured_key_ids IS NULL OR configured_secrets IS NULL
     OR pg_catalog.cardinality(configured_key_ids) = 0
     OR pg_catalog.cardinality(configured_key_ids) <> pg_catalog.cardinality(configured_secrets)
     OR EXISTS (
       SELECT 1 FROM pg_catalog.unnest(configured_key_ids) AS configured(key_id)
       WHERE configured.key_id IS NULL
          OR configured.key_id !~ '^FT_C1_HMAC_[A-Z0-9_]{1,48}$'
     )
     OR EXISTS (
       SELECT 1 FROM pg_catalog.unnest(configured_secrets) AS configured(secret)
       WHERE configured.secret IS NULL OR configured.secret !~ '^[A-Za-z0-9_-]{43}$'
     )
     OR (
       SELECT pg_catalog.count(*) <> pg_catalog.count(DISTINCT configured.key_id)
  ...
  LEFT JOIN public.outcome_comparison_attestation_key_revocations AS revocation
    ON revocation.key_id = activation.key_id
  ORDER BY activation.activation_epoch DESC LIMIT 1;
  IF active_key_id IS NULL OR active_key_id <> configured_active_key_id OR active_revoked THEN
    ok := false;
  ...
  FOR input_index IN 1..pg_catalog.cardinality(configured_key_ids) LOOP
    ...
    IF registry_hash IS NULL OR registry_algorithm <> 'HMAC-SHA-256'
       OR registry_revoked OR registry_hash <> expected_hash THEN
      ok := false;
      diagnostics := pg_catalog.array_append(diagnostics, 'CONFIGURED_KEY_INVALID');
    END IF;
  END LOOP;
  SELECT pg_catalog.count(*)::INTEGER INTO registered_nonrevoked_count
  FROM public.outcome_comparison_attestation_keys AS key_row
  LEFT JOIN public.outcome_comparison_attestation_key_revocations AS revocation
    ON revocation.key_id = key_row.key_id
  ...
  END IF;
  IF EXISTS (
    SELECT 1 FROM public.outcome_comparison_privacy_releases AS release_row
    LEFT JOIN public.outcome_comparison_attestation_key_revocations AS revocation
