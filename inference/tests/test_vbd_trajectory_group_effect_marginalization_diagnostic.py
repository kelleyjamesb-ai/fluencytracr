...
def test_marginalization_extends_only_the_clean_implementation_source_manifest():
    new_paths = {
        ...
        "inference/src/fluencytracr_inference/"
        "vbd_trajectory_group_effect_marginalization_projection.py",
    }
    assert new_paths <= set(validation_runner._IMPLEMENTATION_ONLY_SOURCE_PATHS)
    assert new_paths.isdisjoint(validation_runner._RUNNER_SOURCE_PATHS_V1)
    assert validation_runner._RUNNER_SOURCE_PATHS == tuple(
        sorted(
            (
                *validation_runner._RUNNER_SOURCE_PATHS_V1,
