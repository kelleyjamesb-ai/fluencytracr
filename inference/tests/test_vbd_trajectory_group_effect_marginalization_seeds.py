...
def test_consumed_v1_lineage_is_frozen_without_identifying_root_paths():
    ...
    assert not any(
        name.endswith("_PATH") or name.endswith("_ROOTS")
        for name in dir(marginalization_constants)
    )
