...
def test_geometry_modules_extend_only_the_future_runner_source_manifest():
    geometry_paths = {
    ...
    assert geometry_paths.isdisjoint(validation_runner._RUNNER_SOURCE_PATHS_V3)
    assert len(validation_runner._RUNNER_SOURCE_PATHS_V1) == 30
    assert len(validation_runner._RUNNER_SOURCE_PATHS_V2) == 34
    assert len(validation_runner._RUNNER_SOURCE_PATHS_V3) == 39
