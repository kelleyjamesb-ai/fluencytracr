#!/usr/bin/env python3
"""Standalone consume-before-import bootstrap for the VBD geometry diagnostic."""
...
_SCOPE = "vbd_group_effect_geometry_diagnostic_v1_nonacceptance_one_launch"
...
_STAGED_NAME = "geometry_diagnostic.staged.json"
_OUTPUT_NAME = "geometry_diagnostic.json"
...
class BootstrapError(RuntimeError):
    """The geometry launch failed closed."""
...
def _validate_manifest(manifest: dict, repo: Path, script: Path) -> tuple[Path, str]:
    ...
    if (
        ...
        or not _REQUIRED_GEOMETRY_SOURCES.issubset(paths)
...
def _validate_launch_records(
    ...
    if _root_names(_WORKSPACE) is not None:
        raise BootstrapError("geometry workspace already exists")
    if _root_names(_LIFECYCLE) != tuple(sorted((_AUTHORIZATION_NAME, _PERMIT_NAME))):
        raise BootstrapError("geometry lifecycle is not ready for one launch")
...
def _execute_child(
    ...
    try:
        ...
        from fluencytracr_inference import (
            vbd_trajectory_group_effect_geometry_authorization as candidate,
        )

        candidate.bootstrap_claimed_vbd_trajectory_group_effect_geometry(
            manifest=manifest,
            authorization_path=authorization_path,
            authorization_commit=authorization_commit,
            command_argv=manifest["command_argv"],
            _bootstrap_token=(
                candidate._VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_BOOTSTRAP_CHILD_TOKEN
            ),
        )
...
def _raise_supervisor_termination(signum: int, _frame) -> None:
    raise BootstrapError(f"geometry supervisor interrupted by signal {signum}")
    ...
    while True:
        ...
        if pid == child_pid:
            if not os.WIFEXITED(status) or os.WEXITSTATUS(status) != 0:
                raise BootstrapError("claimed geometry child failed")
            return
        if time.monotonic() >= deadline:
            try:
                os.kill(child_pid, signal.SIGKILL)
                ...
                os.waitpid(child_pid, 0)
            raise BootstrapError("claimed geometry child exceeded compiled timeout")
        time.sleep(0.1)
...
def _validate_persisted(
    ...
    *,
    ...
    _revalidate_source_binding(manifest, authorization_commit, repo)
    publication_token = (
        candidate._VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_PUBLICATION_TOKEN
        if allow_pending_alias
        else None
    )
    candidate.validate_vbd_trajectory_group_effect_geometry_persisted_output(
        path,
        manifest=manifest,
        authorization_commit=authorization_commit,
...
def _publish(
    ...
    if _root_names(_WORKSPACE) != (_STAGED_NAME,):
        raise BootstrapError("staged geometry workspace is off plan")
    ...
    if not stage_removed or _root_names(_WORKSPACE) != (_OUTPUT_NAME,):
        raise BootstrapError("final geometry workspace is off plan")
    try:
