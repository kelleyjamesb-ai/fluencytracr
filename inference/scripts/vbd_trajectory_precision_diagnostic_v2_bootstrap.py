...
TARGET_MODULE = (
    "fluencytracr_inference.vbd_trajectory_precision_diagnostic_v2_authorization"
)
TARGET_FUNCTION = "bootstrap_claimed_vbd_precision_diagnostic_v2"
FAILURE_SCHEMA = "FT_AI_VALUE_VBD_PRECISION_DIAGNOSTIC_BOOTSTRAP_FAILURE_2026_07_V2"
EXECUTION_AUTHORIZATION_SCHEMA = (
    "FT_AI_VALUE_VBD_PRECISION_DIAGNOSTIC_EXECUTION_AUTHORIZATION_2026_07_V2"
)
CLAIM_SCHEMA = "FT_AI_VALUE_VBD_PRECISION_DIAGNOSTIC_ATTEMPT_CLAIM_2026_07_V2"
AUTHORIZATION_SCOPE = "vbd_precision_design_diagnostic_v2_nonacceptance_one_launch"
...
def _validate_manifest_shape(manifest: object) -> dict:
    ...
    if (
        ...
        or manifest["implementation_commit"] == CONSUMED_V1_IMPLEMENTATION_COMMIT
        or not checkpoint_root.is_absolute()
        or checkpoint_root != Path(os.path.normpath(str(checkpoint_root)))
        or manifest["checkpoint_root_identity_hash"]
        != _sha256_bytes(_canonical_bytes(str(checkpoint_root)))
        or workspace == Path(CONSUMED_V1_WORKSPACE_PATH)
        or claim_root == Path(CONSUMED_V1_CLAIM_ROOT_PATH)
        or checkpoint_root
        in {Path(CONSUMED_V1_WORKSPACE_PATH), Path(CONSUMED_V1_CLAIM_ROOT_PATH)}
    ):
...
def _validate_execution_authorization(
    ...
    if (
        ...
        or authorization_commit == CONSUMED_V1_AUTHORIZATION_COMMIT
        or value["execution_authorization_hash"]
        == CONSUMED_V1_EXECUTION_AUTHORIZATION_HASH
    ):
...
def _consume_claim(
    *, manifest: dict, execution_authorization: dict, authorization_commit: str
    ...
    if claim["claim_hash"] == CONSUMED_V1_CLAIM_HASH:
        raise BootstrapError("consumed V1 claim cannot satisfy V2")
    descriptor = -1
    try:
        descriptor = os.open(
            CLAIM_FILENAME,
    ...
    *,
    ...
    try:
        ...
        bootstrap_token = getattr(
            module, "_VBD_PRECISION_DIAGNOSTIC_V2_BOOTSTRAP_CHILD_TOKEN"
        )
        function(
            manifest=manifest,
