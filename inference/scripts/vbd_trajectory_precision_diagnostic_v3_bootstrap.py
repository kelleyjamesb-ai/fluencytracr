...
TARGET_MODULE = (
    "fluencytracr_inference.vbd_trajectory_precision_diagnostic_v3_authorization"
)
TARGET_FUNCTION = "bootstrap_claimed_vbd_precision_diagnostic_v3"
VALIDATION_FUNCTION = "validate_vbd_precision_diagnostic_v3_persisted_output"
MANIFEST_VALIDATION_FUNCTION = (
    "validate_vbd_precision_diagnostic_v3_authorization_manifest"
)
FAILURE_SCHEMA = "FT_AI_VALUE_VBD_PRECISION_DIAGNOSTIC_BOOTSTRAP_FAILURE_2026_07_V3"
EXECUTION_AUTHORIZATION_SCHEMA = (
    "FT_AI_VALUE_VBD_PRECISION_DIAGNOSTIC_EXECUTION_AUTHORIZATION_2026_07_V3"
)
CLAIM_SCHEMA = "FT_AI_VALUE_VBD_PRECISION_DIAGNOSTIC_ATTEMPT_CLAIM_2026_07_V3"
...
CONSUMED_V2_CLAIM_ROOT_PATH = (
    "/Users/jameskelley/.codex/evidence/vbd-mcse-diagnostic-v2-claim"
)
CONSUMED_V2_CHECKPOINT_ROOT_PATH = (
    "/Users/jameskelley/.codex/evidence/vbd-mcse-diagnostic-v2-checkpoints"
)
V3_WORKSPACE_PATH = (
...
V3_CHECKPOINT_ROOT_PATH = (
    "/Users/jameskelley/.codex/evidence/vbd-mcse-diagnostic-v3-checkpoints"
)
V3_DIAGNOSTIC_PLAN_HASH = (
    "ddd11d943296324ea48c5738a05c4e7c123cd03321c28440a33bedd9800ba8ef"
)
V3_SEED_MANIFEST_HASH = (
    "70028050634e919ad671440d3f2918808b5d3f78f3fce168fdfed8042154c8d4"
)
...
def _validate_manifest_shape(manifest: object) -> dict:
    ...
    checkpoint_root = Path(manifest.get("checkpoint_root_path", ""))
    workspace = Path(manifest.get("canonical_workspace_path", ""))
    claim_root = Path(manifest.get("external_claim_root_path", ""))
    authorization_path = Path(
        manifest.get("execution_authorization_record_path", "")
    )
    ...
    if any(
        ...
        or _SHA256_RE.fullmatch(manifest[key]) is None
        for key in hash_fields
    ):
        raise BootstrapError("manifest hash field is invalid")
    implementation_commit = manifest.get("implementation_commit")
    implementation_tree = manifest.get("implementation_tree")
    reviews = manifest.get("implementation_review_refs")
    ...
    if (
        ...
        or _COMMIT_RE.fullmatch(implementation_tree) is None
        or type(reviews) is not dict
        or set(reviews) != set(expected_review_roles)
        or len(set(reviews.values())) != len(expected_review_roles)
        or any(
            type(reviews[role]) is not str
            or _REVIEW_REF_RE.fullmatch(reviews[role]) is None
            or not reviews[role].startswith(
                f"review:{slug}/go/{implementation_commit}/"
            )
            for role, slug in expected_review_roles.items()
        )
    ):
        raise BootstrapError("manifest implementation review binding is invalid")
    files = manifest.get("in_scope_files")
    if (
        ...
        or manifest["in_scope_files_hash"]
        != _sha256_bytes(_canonical_bytes(files))
    ):
        raise BootstrapError("manifest source binding is invalid")
    if (
        ...
        or manifest["diagnostic_plan_hash"] != V3_DIAGNOSTIC_PLAN_HASH
        ...
        or output_path.name != "diagnostic.json"
        or manifest["canonical_workspace_identity_hash"]
        != _sha256_bytes(_canonical_bytes(str(workspace)))
        or manifest["external_claim_root_identity_hash"]
        != _sha256_bytes(_canonical_bytes(str(claim_root)))
        or manifest["checkpoint_root_identity_hash"]
        != _sha256_bytes(_canonical_bytes(str(checkpoint_root)))
        or manifest["execution_authorization_record_path_hash"]
        != _sha256_bytes(_canonical_bytes(str(authorization_path)))
        or manifest["output_path_hash"]
        != _sha256_bytes(_canonical_bytes(str(output_path)))
        or workspace != Path(V3_WORKSPACE_PATH)
        or claim_root != Path(V3_CLAIM_ROOT_PATH)
        or checkpoint_root != Path(V3_CHECKPOINT_ROOT_PATH)
        or any(
            left == right
            ...
            for right in (
                Path(CONSUMED_V1_WORKSPACE_PATH),
                Path(CONSUMED_V1_CLAIM_ROOT_PATH),
                Path(CONSUMED_V2_WORKSPACE_PATH),
                Path(CONSUMED_V2_CLAIM_ROOT_PATH),
                Path(CONSUMED_V2_CHECKPOINT_ROOT_PATH),
            )
        )
    ):
...
def _reject_symlinked_path(path: Path, label: str) -> None:
    ...
    for part in path.parts[1:]:
        ...
        if stat.S_ISLNK(info.st_mode):
            raise BootstrapError(f"{label} resolves through a symlink")


def _existing_directory_names(path: Path, label: str) -> tuple[str, ...]:
...
def _preflight_fixed_roots(manifest: dict) -> None:
    ...
    if _existing_directory_names(workspace, "diagnostic workspace"):
        raise BootstrapError("diagnostic workspace is not empty before execution")
    if _existing_directory_names(checkpoint_root, "diagnostic checkpoint root"):
        raise BootstrapError(
            "diagnostic checkpoint root is not empty before execution"
        )
    if set(_existing_directory_names(claim_root, "diagnostic claim root")) != {
        authorization_path.name
    }:
        raise BootstrapError("diagnostic claim root contents are off-plan")
    _reject_symlinked_path(authorization_path, "execution authorization")
    try:
        info = authorization_path.lstat()
    except OSError as exc:
        raise BootstrapError("execution authorization is unavailable") from exc
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
        raise BootstrapError("execution authorization is unsafe")
    ...
    for value in site_paths:
        if type(value) is not str:
            raise BootstrapError("site-package path is invalid")
        path = Path(value)
        _reject_symlinked_path(path, "site-package path")
...
def _validate_execution_authorization(
    ...
    if (
        ...
        in {CONSUMED_V1_AUTHORIZATION_COMMIT, CONSUMED_V2_AUTHORIZATION_COMMIT}
        or value["execution_authorization_hash"]
        in {
            CONSUMED_V1_EXECUTION_AUTHORIZATION_HASH,
            CONSUMED_V2_EXECUTION_AUTHORIZATION_HASH,
...
def _consume_claim(
    *, manifest: dict, execution_authorization: dict, authorization_commit: str
    ...
    if claim["claim_hash"] in {CONSUMED_V1_CLAIM_HASH, CONSUMED_V2_CLAIM_HASH}:
        raise BootstrapError("consumed V1/V2 claim cannot satisfy V3")
    descriptor = -1
    try:
        descriptor = os.open(
            CLAIM_FILENAME,
    ...
    *,
    ...
    try:
        ...
        bootstrap_token = getattr(
            module, "_VBD_PRECISION_DIAGNOSTIC_V3_BOOTSTRAP_CHILD_TOKEN"
        )
        function(
            manifest=manifest,
...
def _execute_validation_child(
    *,
    ...
    try:
        _configure_numerical_environment()
        _install_reviewed_source(modules, manifest)
        module = __import__(TARGET_MODULE, fromlist=[VALIDATION_FUNCTION])
        function = getattr(module, VALIDATION_FUNCTION)
        function(
            manifest=manifest,
            execution_authorization=execution_authorization,
            claim=claim,
...
def _execute_manifest_validation_child(*, modules: dict, manifest: dict) -> None:
    try:
        _configure_numerical_environment()
        _install_reviewed_source(modules, manifest)
        module = __import__(TARGET_MODULE, fromlist=[MANIFEST_VALIDATION_FUNCTION])
        function = getattr(module, MANIFEST_VALIDATION_FUNCTION)
        function(manifest)
    except BaseException:
        os._exit(2)
    os._exit(0)
...
def _read_canonical_staged_output_bytes(
    ...
    try:
        ...
        os.close(descriptor)
    after_name = os.stat(filename, dir_fd=workspace_fd, follow_symlinks=False)
    identities = {
...
def _read_canonical_staged_output(
    workspace_fd: int, filename: str = STAGED_OUTPUT_FILENAME
) -> object:
    return _read_canonical_staged_output_bytes(workspace_fd, filename)[0]
...
def _validate_persisted_in_child(
    *,
    modules: dict,
    manifest: dict,
    execution_authorization: dict,
    ...
    try:
        ...
        raise BootstrapError("diagnostic validation child could not start") from exc
    if child_pid == 0:
        _execute_validation_child(
            modules=modules,
            manifest=manifest,
            execution_authorization=execution_authorization,
            claim=claim,
...
def _validate_manifest_in_child(*, modules: dict, manifest: dict) -> None:
    try:
        child_pid = os.fork()
    except OSError as exc:
        raise BootstrapError("manifest validation child could not start") from exc
    if child_pid == 0:
        _execute_manifest_validation_child(modules=modules, manifest=manifest)
    _wait_for_claimed_child(child_pid)


def _preflight_and_consume_claim(
    *,
    modules: dict,
    manifest: dict,
    execution_authorization: dict,
    ...
    _validate_manifest_in_child(modules=modules, manifest=manifest)
    _preflight_fixed_roots(manifest)
    return _consume_claim(
        manifest=manifest,
        execution_authorization=execution_authorization,
        authorization_commit=authorization_commit,
    )
...
def _publish_staged_output(
    ...
    workspace_fd, output = _open_output_workspace(manifest)
    try:
        value, staged_bytes = _read_canonical_staged_output_bytes(workspace_fd)
        if (
            expected_staged_bytes is not None
...
def _supervise_and_publish(
    ...
    *,
    ...
    published = False
    try:
        workspace_fd, _output = _open_output_workspace(manifest)
        try:
            staged_value, staged_bytes = _read_canonical_staged_output_bytes(
                workspace_fd
            )
        finally:
            os.close(workspace_fd)
        _validate_persisted_in_child(
            modules=modules,
            manifest=manifest,
            execution_authorization=execution_authorization,
            claim=claim,
        ...
        published_value = _publish_staged_output(
            manifest, expected_staged_bytes=staged_bytes
        )
        published = True
        if published_value != staged_value:
            raise BootstrapError("diagnostic published value differs from staging")
        _validate_persisted_in_child(
            modules=modules,
            manifest=manifest,
            execution_authorization=execution_authorization,
...
def _run(authorization_path: Path) -> int:
    ...
    claim = _preflight_and_consume_claim(
        modules=modules,
        manifest=manifest,
        execution_authorization=execution_authorization,
        authorization_commit=authorization_commit,
