"""Authorization and one-shot lifecycle for the VBD geometry diagnostic."""
...
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_AUTHORIZATION_MANIFEST_RELATIVE_PATH = (
    "inference/evidence/vbd_trajectory_group_effect_geometry_authorization.json"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_BOOTSTRAP_RELATIVE_PATH = (
    "inference/scripts/vbd_trajectory_group_effect_geometry_bootstrap.py"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_AUTHORIZATION_MANIFEST_SCHEMA_VERSION = (
    "FT_AI_VALUE_VBD_GROUP_EFFECT_GEOMETRY_AUTHORIZATION_MANIFEST_2026_07_V1"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_EXECUTION_AUTHORIZATION_SCHEMA_VERSION = (
    "FT_AI_VALUE_VBD_GROUP_EFFECT_GEOMETRY_EXECUTION_AUTHORIZATION_2026_07_V1"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_LAUNCH_PERMIT_SCHEMA_VERSION = (
    "FT_AI_VALUE_VBD_GROUP_EFFECT_GEOMETRY_LAUNCH_PERMIT_2026_07_V1"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_CLAIM_SCHEMA_VERSION = (
    "FT_AI_VALUE_VBD_GROUP_EFFECT_GEOMETRY_ATTEMPT_CLAIM_2026_07_V1"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_INPUT_BINDING_SCHEMA_VERSION = (
    "FT_AI_VALUE_VBD_GROUP_EFFECT_GEOMETRY_INPUT_BINDING_2026_07_V1"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_COMPLETION_RECEIPT_SCHEMA_VERSION = (
    "FT_AI_VALUE_VBD_GROUP_EFFECT_GEOMETRY_COMPLETION_RECEIPT_2026_07_V1"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_AUTHORIZATION_SCOPE = (
    "vbd_group_effect_geometry_diagnostic_v1_nonacceptance_one_launch"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_EXECUTION_AUTHORIZATION_FILENAME = (
    "execution_authorization.json"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_LAUNCH_PERMIT_FILENAME = (
    "launch_permit.json"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_CONSUMED_PERMIT_FILENAME = (
    "launch_permit.consumed.json"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_CLAIM_FILENAME = "attempt_claim.json"
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_INPUT_BINDING_FILENAME = "input_binding.json"
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_COMPLETION_RECEIPT_FILENAME = (
    "completion_receipt.json"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_OUTPUT_FILENAME = (
    "geometry_diagnostic.json"
)
VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_STAGED_OUTPUT_FILENAME = (
    "geometry_diagnostic.staged.json"
)
...
_DECISION_REF_RE = re.compile(
    r"^human-authorization:vbd-geometry-diagnostic/[A-Za-z0-9._-]+$"
)
_VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_EXECUTION_TOKEN = object()
_VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_BOOTSTRAP_CHILD_TOKEN = object()
_VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_PUBLICATION_TOKEN = object()


class VbdTrajectoryGroupEffectGeometryAuthorizationError(RuntimeError):
    """The geometry diagnostic authorization lifecycle is invalid."""


def _authorization_error(message: str, exc: Exception | None = None):
    error = VbdTrajectoryGroupEffectGeometryAuthorizationError(message)
    if exc is None:
        raise error
...
def _strict_timestamp(value: object) -> str:
    if type(value) is not str or _UTC_RE.fullmatch(value) is None:
        _authorization_error("geometry authorization timestamp is invalid")
    try:
        datetime.strptime(value, "%Y-%m-%dT%H:%M:%SZ")
    except ValueError as exc:
        _authorization_error("geometry authorization timestamp is invalid", exc)
    return value
...
def _open_absolute_directory_no_symlinks(
    ...
    *,
    ...
    path = _strict_absolute_path(str(path), "geometry directory")
...
def _strict_pairs(pairs: list[tuple[str, object]]) -> dict:
    ...
    for key, item in pairs:
        if key in value:
            _authorization_error("geometry lifecycle JSON has duplicate keys")
        value[key] = item
    return value
...
def _read_canonical_json(path: Path, label: str) -> dict:
    ...
    except VbdTrajectoryGroupEffectGeometryAuthorizationError:
        raise
...
def _validate_in_scope_files(value: object) -> list[dict]:
    if type(value) is not list:
        _authorization_error("geometry in-scope files must be a list")
    expected_paths = sorted(_RUNNER_SOURCE_PATHS)
    paths = []
    for item in value:
        if type(item) is not dict or set(item) != {"path", "sha256"}:
            _authorization_error("geometry in-scope file entry is invalid")
        path = item["path"]
        if (
            ...
            _authorization_error("geometry in-scope path is unsafe")
        _strict_hash(item["sha256"], "geometry in-scope file hash")
        paths.append(path)
    if paths != expected_paths or len(set(paths)) != len(paths):
        _authorization_error("geometry in-scope source set is incomplete")
    return value
    ...
    "CLAIMED": (
        {
            VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_EXECUTION_AUTHORIZATION_FILENAME,
            VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_CONSUMED_PERMIT_FILENAME,
            VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_CLAIM_FILENAME,
        },
        None,
    ),
    "INPUT_BOUND": (
        {
            ...
            VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_CONSUMED_PERMIT_FILENAME,
            VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_CLAIM_FILENAME,
            VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_INPUT_BINDING_FILENAME,
        },
        None,
    ),
    "COMPLETION_RECORDED": (
...
def preflight_vbd_trajectory_group_effect_geometry_fixed_roots(
    *, manifest: dict, phase: str
    ...
    if phase not in _ROOT_PHASE_FILES:
        _authorization_error("geometry root phase is invalid")
    lifecycle_files, workspace_files = _ROOT_PHASE_FILES[phase]
    paths = _fixed_paths(manifest)
    _validate_root_files(paths["lifecycle"], lifecycle_files, "geometry lifecycle root")
...
def validate_vbd_trajectory_group_effect_geometry_authorization_manifest(
    value: object,
) -> dict:
    if type(value) is not dict or set(value) != _manifest_keys():
        _authorization_error("geometry authorization manifest shape is invalid")
    ...
    implementation_commit = _strict_commit(
        ...
        "geometry implementation commit",
    )
    _strict_commit(value["implementation_tree"], "geometry implementation tree")
    ...
    if not _implementation_review_refs_are_valid(
        ...
        _authorization_error("geometry implementation review refs are invalid")
    for key in (
        "in_scope_files_hash",
        "runtime_identity_hash",
        "requirements_lock_hash",
    ...
    if (
        ...
        or paths["authorization"] != lifecycle / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_EXECUTION_AUTHORIZATION_FILENAME
        or paths["permit"] != lifecycle / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_LAUNCH_PERMIT_FILENAME
        or paths["consumed_permit"] != lifecycle / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_CONSUMED_PERMIT_FILENAME
        or paths["claim"] != lifecycle / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_CLAIM_FILENAME
        or paths["input"] != lifecycle / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_INPUT_BINDING_FILENAME
        or paths["completion"] != lifecycle / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_COMPLETION_RECEIPT_FILENAME
        or paths["output"].parent != workspace
        or paths["output"].name
        != VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_OUTPUT_FILENAME
        or workspace
        != Path(VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_WORKSPACE_PATH)
        or lifecycle
        != Path(VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_LIFECYCLE_ROOT_PATH)
        ...
        _authorization_error("geometry authorization manifest is invalid")
    ...
    if (
        ...
        _authorization_error("geometry site-package paths are invalid")
    if not git_executable.is_absolute():
        _authorization_error("geometry Git executable is invalid")
    if any(
        type(item) is str
        and any(
            token in item.lower()
        ...
        _authorization_error("geometry manifest contains unsafe scope")
    return value


def build_vbd_trajectory_group_effect_geometry_authorization_manifest(
    *,
    ...
    if _git_output("status", "--porcelain=v1", "--untracked-files=all"):
        _authorization_error("geometry implementation checkout must be clean")
    ...
    workspace = Path(VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_WORKSPACE_PATH)
    lifecycle = Path(VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_LIFECYCLE_ROOT_PATH)
    authorization = lifecycle / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_EXECUTION_AUTHORIZATION_FILENAME
    permit = lifecycle / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_LAUNCH_PERMIT_FILENAME
    consumed = lifecycle / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_CONSUMED_PERMIT_FILENAME
    claim = lifecycle / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_CLAIM_FILENAME
    input_binding = lifecycle / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_INPUT_BINDING_FILENAME
    completion = lifecycle / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_COMPLETION_RECEIPT_FILENAME
    output = workspace / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_OUTPUT_FILENAME
    bootstrap = (
        _repo_root()
        / VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_BOOTSTRAP_RELATIVE_PATH
    ).resolve()
    ...
    return validate_vbd_trajectory_group_effect_geometry_authorization_manifest(
...
def _permit_body(
    *,
    ...
    return {
        ...
        "scope": VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_AUTHORIZATION_SCOPE,
...
def validate_vbd_trajectory_group_effect_geometry_launch_permit(
    value: object,
    *,
    manifest: dict,
    authorization_commit: str,
    ...
    _strict_commit(authorization_commit, "geometry authorization commit")
    expected_keys = set(
        _permit_body(
            manifest=manifest,
            authorization_commit=authorization_commit,
    ...
    if type(value) is not dict or set(value) != expected_keys:
        _authorization_error("geometry launch permit shape is invalid")
    token = value["permit_token"]
    if type(token) is not str or _TOKEN_RE.fullmatch(token) is None:
        _authorization_error("geometry launch permit token is invalid")
    body = _permit_body(
        manifest=manifest,
        authorization_commit=authorization_commit,
        permit_token=token,
    ...
    if value != expected:
        _authorization_error("geometry launch permit is invalid")
    return value


def create_vbd_trajectory_group_effect_geometry_launch_permit(
    *,
    manifest: dict,
    authorization_commit: str,
) -> dict:
    ...
    _write_exclusive_json(path, permit, "geometry launch permit")
    preflight_vbd_trajectory_group_effect_geometry_fixed_roots(
        manifest=manifest, phase="PERMIT_AVAILABLE"
    )
    return validate_vbd_trajectory_group_effect_geometry_launch_permit(
        permit,
        manifest=manifest,
        authorization_commit=authorization_commit,
    )
...
def build_vbd_trajectory_group_effect_geometry_execution_authorization(
    *,
    manifest: dict,
    authorization_commit: str,
    ...
    permit = validate_vbd_trajectory_group_effect_geometry_launch_permit(
        _read_canonical_json(permit_path, "geometry launch permit"),
        manifest=manifest,
        authorization_commit=authorization_commit,
    )
    ...
    if (
        ...
        _authorization_error("geometry human decision reference is invalid")
    _strict_hash(decision_text_hash, "geometry decision text hash")
    body = {
        "schema_version": (
            VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_EXECUTION_AUTHORIZATION_SCHEMA_VERSION
        ),
...
def validate_vbd_trajectory_group_effect_geometry_execution_authorization(
    value: object,
    *,
    manifest: dict,
    authorization_commit: str,
    ...
    if type(value) is not dict or set(value) != expected_keys:
        _authorization_error("geometry execution authorization shape is invalid")
    body = {
        key: item
    ...
    for key in (
        ...
        _strict_hash(value[key], f"geometry {key}")
    if (
        value["schema_version"]
        != VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_EXECUTION_AUTHORIZATION_SCHEMA_VERSION
        ...
        _authorization_error("geometry execution authorization is invalid")
    return value


def write_vbd_trajectory_group_effect_geometry_execution_authorization(
    *,
    manifest: dict,
    authorization: dict,
    authorization_commit: str,
) -> None:
    authorization = validate_vbd_trajectory_group_effect_geometry_execution_authorization(
        authorization,
        manifest=manifest,
        authorization_commit=authorization_commit,
    )
...
def validate_vbd_trajectory_group_effect_geometry_claim(
    value: object,
    *,
    manifest: dict,
    execution_authorization: dict,
    ...
    if value != expected:
        _authorization_error("geometry attempt claim is invalid")
    return value


def read_vbd_trajectory_group_effect_geometry_consumed_permit(
    *,
    manifest: dict,
    execution_authorization: dict,
    authorization_commit: str,
    ...
    permit = validate_vbd_trajectory_group_effect_geometry_launch_permit(
        _read_canonical_json(path, "geometry consumed permit"),
        manifest=manifest,
        authorization_commit=authorization_commit,
    )
    device, inode, file_hash = _regular_file_identity(
    ...
    if (
        ...
        _authorization_error("geometry consumed permit identity differs")
    return permit


def read_vbd_trajectory_group_effect_geometry_execution_authorization(
    path: Path,
    *,
    manifest: dict,
    authorization_commit: str,
    ...
    if path.resolve() != expected:
        _authorization_error("geometry execution authorization path is off-plan")
    return validate_vbd_trajectory_group_effect_geometry_execution_authorization(
        _read_canonical_json(path, "geometry execution authorization"),
        manifest=manifest,
        authorization_commit=authorization_commit,
    )


def read_vbd_trajectory_group_effect_geometry_claim(
    *,
    manifest: dict,
    execution_authorization: dict,
    authorization_commit: str,
) -> dict:
    return validate_vbd_trajectory_group_effect_geometry_claim(
        _read_canonical_json(
            Path(manifest["claim_path"]),
            "geometry attempt claim",
        ),
        manifest=manifest,
        execution_authorization=execution_authorization,
        authorization_commit=authorization_commit,
...
def build_vbd_trajectory_group_effect_geometry_input_binding(
    *,
    manifest: dict,
    claim: dict,
    ...
    if (
        ...
        _authorization_error("geometry input claim is invalid")
    body = {
        "schema_version": (
            VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_INPUT_BINDING_SCHEMA_VERSION
        ),
        ...
        "case_panel_hashes": _hash_list(
            case_panel_hashes, 2, "case panel"
        ),
        "ordered_panel_manifest_roots": _hash_list(
            ordered_panel_manifest_roots, 2, "ordered panel manifest"
        ),
        "prepared_input_hashes": _hash_list(
            prepared_input_hashes, 6, "prepared input"
        ),
        "model_input_hashes": _hash_list(
            model_input_hashes, 6, "model input"
        ),
    ...
    if (
        ...
        != [
            ...
            for item in vbd_trajectory_group_effect_geometry_plan()[
                "execution_order"
            ]
        ]
    ):
        _authorization_error("geometry deterministic or sampler binding differs")
    return {**body, "input_binding_hash": sha256_json(body)}


def validate_vbd_trajectory_group_effect_geometry_input_binding(
    value: object,
    *,
    manifest: dict,
    claim: dict,
    ...
    if type(value) is not dict or "input_binding_hash" not in value:
        _authorization_error("geometry input binding shape is invalid")
    rebuilt = build_vbd_trajectory_group_effect_geometry_input_binding(
        manifest=manifest,
        claim=claim,
    ...
    if value != rebuilt:
        _authorization_error("geometry input binding differs")
    return value


def write_vbd_trajectory_group_effect_geometry_input_binding(
    *,
    manifest: dict,
    binding: dict,
    claim: dict,
) -> None:
    binding = validate_vbd_trajectory_group_effect_geometry_input_binding(
        binding,
        manifest=manifest,
        claim=claim,
    )
...
def read_vbd_trajectory_group_effect_geometry_input_binding(
    *,
    manifest: dict,
    claim: dict,
) -> dict:
    return validate_vbd_trajectory_group_effect_geometry_input_binding(
        _read_canonical_json(
            Path(manifest["input_binding_path"]),
            "geometry input binding",
        ),
...
def build_vbd_trajectory_group_effect_geometry_completion_receipt(
    *,
    manifest: dict,
    claim: dict,
    input_binding: dict,
    ...
    if type(arm_records) is not list or len(arm_records) != 12:
        _authorization_error("geometry completion arm matrix is incomplete")
    arm_hashes = _hash_list(
        [record.get("arm_record_hash") for record in arm_records],
        12,
        "arm record",
    )
    body = {
        ...
        "diagnostic_plan_hash": manifest["diagnostic_plan_hash"],
        "seed_manifest_hash": manifest["seed_manifest_hash"],
        "ordered_arm_record_hashes": arm_hashes,
        "ordered_arm_record_hashes_hash": sha256_json(arm_hashes),
...
def validate_vbd_trajectory_group_effect_geometry_completion_receipt(
    value: object,
    *,
    manifest: dict,
    claim: dict,
    ...
    rebuilt = build_vbd_trajectory_group_effect_geometry_completion_receipt(
        manifest=manifest,
        claim=claim,
    ...
    if value != rebuilt:
        _authorization_error("geometry completion receipt differs")
    return value


def write_vbd_trajectory_group_effect_geometry_completion_receipt(
    *,
    manifest: dict,
    receipt: dict,
    claim: dict,
    ...
    receipt = validate_vbd_trajectory_group_effect_geometry_completion_receipt(
        receipt,
        manifest=manifest,
        claim=claim,
        input_binding=input_binding,
...
def read_vbd_trajectory_group_effect_geometry_completion_receipt(
    *,
    manifest: dict,
    claim: dict,
    input_binding: dict,
    ...
    return validate_vbd_trajectory_group_effect_geometry_completion_receipt(
        _read_canonical_json(
            Path(manifest["completion_receipt_path"]),
            "geometry completion receipt",
        ),
...
def write_vbd_trajectory_group_effect_geometry_staged_output(
    *,
    manifest: dict,
    record: dict,
) -> None:
    record = validate_vbd_trajectory_group_effect_geometry_record(record)
    if record["execution_state"] != "COMPLETE":
        _authorization_error("unexecuted geometry record cannot be staged")
    preflight_vbd_trajectory_group_effect_geometry_fixed_roots(
        manifest=manifest, phase="COMPLETION_RECORDED"
    )
...
def _validate_vbd_trajectory_group_effect_geometry_arm_input_bindings(
    *,
    arm_records: list[dict],
    input_binding: dict,
) -> None:
    ...
    if input_binding["expected_sampler_binding_hashes"] != expected_sampler_hashes:
        _authorization_error("geometry persisted sampler plan differs")
    for arm_index, arm in enumerate(arm_records):
        binding = arm["binding"]
        ...
        if (
            binding["binding_hash"] != expected_sampler_hashes[arm_index]
            or arm["panel_hash"]
            != input_binding["case_panel_hashes"][case_ordinal]
            or arm["ordered_panel_manifest_root"]
            != input_binding["ordered_panel_manifest_roots"][
                case_ordinal
            ]
            or arm["prepared_input_hash"]
            != input_binding["prepared_input_hashes"][lane_index]
            or arm["model_input_hash"]
            != input_binding["model_input_hashes"][lane_index]
            or arm["deterministic_reference_hash"]
            != input_binding["deterministic_reference_hashes"][lane_index]
            or arm["deterministic_recomputation_hash"]
            != input_binding["deterministic_recomputation_hashes"][lane_index]
        ):
            _authorization_error("geometry persisted arm input binding differs")


def validate_vbd_trajectory_group_effect_geometry_persisted_output(
    path: Path,
    *,
    manifest: dict,
    authorization_commit: str,
    ...
    if resolved_path == staged_path and _publication_token is None:
        ...
        is _VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_PUBLICATION_TOKEN
    ):
        ...
        _authorization_error("geometry publication token is invalid")
    else:
        _authorization_error("geometry persisted output path is off-plan")
    preflight_vbd_trajectory_group_effect_geometry_fixed_roots(
        manifest=manifest,
        phase=root_phase,
    )
    record = validate_vbd_trajectory_group_effect_geometry_record(
        _read_canonical_json(path, "geometry persisted output")
    )
    if record["execution_state"] != "COMPLETE":
        _authorization_error("geometry persisted output is unexecuted")
    authorization = read_vbd_trajectory_group_effect_geometry_execution_authorization(
        Path(manifest["execution_authorization_record_path"]),
        manifest=manifest,
        authorization_commit=authorization_commit,
    )
    read_vbd_trajectory_group_effect_geometry_consumed_permit(
        manifest=manifest,
        execution_authorization=authorization,
        authorization_commit=authorization_commit,
    )
    claim = read_vbd_trajectory_group_effect_geometry_claim(
        manifest=manifest,
        execution_authorization=authorization,
        authorization_commit=authorization_commit,
    )
    input_binding = read_vbd_trajectory_group_effect_geometry_input_binding(
        manifest=manifest,
        claim=claim,
    )
    receipt = read_vbd_trajectory_group_effect_geometry_completion_receipt(
        manifest=manifest,
        claim=claim,
        input_binding=input_binding,
        arm_records=record["arm_records"],
    )
    ...
    if any(completion[key] != value for key, value in expected.items()):
        _authorization_error("geometry completion provenance differs")
    if completion["ordered_arm_record_hashes_hash"] != receipt[
        "ordered_arm_record_hashes_hash"
    ]:
        _authorization_error("geometry completion arm root differs")
    _validate_vbd_trajectory_group_effect_geometry_arm_input_bindings(
        arm_records=record["arm_records"],
        input_binding=input_binding,
    )
    info = path.lstat()
    if (
        ...
        _authorization_error("geometry persisted output publication is incomplete")
    return record


def verify_vbd_trajectory_group_effect_geometry_authorization_commit(
    *,
    manifest: dict,
    authorization_commit: str,
) -> None:
    ...
    authorization_commit = _strict_commit(
        authorization_commit,
        "geometry authorization commit",
    )
    if _git_output("status", "--porcelain=v1", "--untracked-files=all"):
        _authorization_error("geometry authorization checkout is dirty")
    if _git_output("rev-parse", "HEAD") != authorization_commit:
        _authorization_error("geometry checkout is not exact A")
    parents = _git_output(
        "rev-list",
        "--parents",
        "-n",
    ...
    if parents != [authorization_commit, manifest["implementation_commit"]]:
        _authorization_error("geometry A is not the sole child of D")
    changed = _git_output(
        "diff-tree",
        "--no-commit-id",
        "--name-only",
    ...
    if changed != [
        ...
        _authorization_error("geometry D to A diff is not manifest-only")
    if _git_output(
        "rev-parse",
        f"{manifest['implementation_commit']}^{{tree}}",
    ) != manifest["implementation_tree"]:
        _authorization_error("geometry implementation tree binding differs")
    ...
    if (
        _read_canonical_json(manifest_path, "geometry committed manifest")
        != manifest
        or _git_output("hash-object", str(manifest_path))
        != _git_output(
            "rev-parse",
            f"{authorization_commit}:{VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_AUTHORIZATION_MANIFEST_RELATIVE_PATH}",
        )
    ):
        _authorization_error("geometry committed manifest bytes differ")


def bootstrap_claimed_vbd_trajectory_group_effect_geometry(
    *,
    manifest: dict,
    ...
    if (
        _bootstrap_token
        is not _VBD_TRAJECTORY_GROUP_EFFECT_GEOMETRY_BOOTSTRAP_CHILD_TOKEN
    ):
        _authorization_error("geometry execution requires bootstrap token")
    manifest = validate_vbd_trajectory_group_effect_geometry_authorization_manifest(
        manifest
    )
    if command_argv != manifest["command_argv"]:
        _authorization_error("geometry bootstrap argv differs")
    preflight_vbd_trajectory_group_effect_geometry_fixed_roots(
    ...
    from .vbd_trajectory_group_effect_geometry_execution import (
        execute_authorized_vbd_trajectory_group_effect_geometry,
    )

    record = execute_authorized_vbd_trajectory_group_effect_geometry(
        manifest=manifest,
        execution_authorization=authorization,
        claim=claim,
        _execution_token=(
    ...
    write_vbd_trajectory_group_effect_geometry_staged_output(
        manifest=manifest,
        record=record,
    )
    return record
