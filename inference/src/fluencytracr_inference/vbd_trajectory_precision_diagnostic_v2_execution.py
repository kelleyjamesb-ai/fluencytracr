"""Private one-shot execution for the permanently held V2 MCSE diagnostic."""
...
from .vbd_trajectory_nuts import (
    ...
    _sample_vbd_trajectory_precision_diagnostic_v2,
...
from .vbd_trajectory_precision_diagnostic_v2_authorization import (
    _VBD_PRECISION_DIAGNOSTIC_V2_EXECUTION_TOKEN,
    bind_vbd_precision_diagnostic_v2_input,
    validate_vbd_precision_diagnostic_v2_authorization_manifest,
    validate_vbd_precision_diagnostic_v2_claim,
    validate_vbd_precision_diagnostic_v2_execution_authorization,
)
...
def _create_empty_checkpoint_root(path: Path) -> None:
    try:
        ...
        raise RuntimeError("precision diagnostic V2 checkpoint root is unavailable") from exc


def execute_authorized_vbd_precision_diagnostic_v2(
    *,
    manifest: dict,
    ...
    """Execute once after the external V2 claim has already been consumed."""

    if _execution_token is not _VBD_PRECISION_DIAGNOSTIC_V2_EXECUTION_TOKEN:
        raise PermissionError("precision diagnostic V2 execution requires its token")
    ...
    try:
        claim = validate_vbd_precision_diagnostic_v2_claim(
            claim,
            manifest=manifest,
        ...
        raise PermissionError("precision diagnostic V2 claim is invalid") from exc
    ...
    if (
        ...
        raise RuntimeError("precision diagnostic V2 runtime or model binding drifted")

    checkpoint_root = Path(manifest["checkpoint_root_path"])
    _create_empty_checkpoint_root(checkpoint_root)
    checkpoint_identity = VbdTrajectoryPrecisionDiagnosticV2CheckpointIdentity(
    ...
    try:
        ...
        input_binding = bind_vbd_precision_diagnostic_v2_input(
            manifest=manifest,
            claim=claim,
        ...
        for lane_ordinal, lane in enumerate(VBD_TRAJECTORY_LANES):
            ...
            idata = _sample_vbd_trajectory_precision_diagnostic_v2(
        ...
        if checkpoints[-1] != terminal:
            raise RuntimeError("precision diagnostic V2 terminal checkpoint drifted")
        provenance = {
            "authorization_commit": authorization_commit,
            "authorization_manifest_hash": manifest["manifest_hash"],
        ...
        return validate_vbd_trajectory_precision_diagnostic_v2_record_with_checkpoints(
            record,
