...
VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_AUTHORIZATION_MANIFEST_SCHEMA_VERSION = (
    "FT_AI_VALUE_VBD_PRECISION_DIAGNOSTIC_AUTHORIZATION_MANIFEST_2026_07_V3"
)
VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_EXECUTION_AUTHORIZATION_SCHEMA_VERSION = (
    "FT_AI_VALUE_VBD_PRECISION_DIAGNOSTIC_EXECUTION_AUTHORIZATION_2026_07_V3"
)
VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_CLAIM_SCHEMA_VERSION = (
    "FT_AI_VALUE_VBD_PRECISION_DIAGNOSTIC_ATTEMPT_CLAIM_2026_07_V3"
)
VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_AUTHORIZATION_SCOPE = (
    "vbd_precision_design_diagnostic_v3_nonacceptance_one_launch"
)
VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_CLAIM_FILENAME = "attempt_claim.json"
VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_INPUT_BINDING_FILENAME = "input_binding.json"
VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_OUTPUT_FILENAME = "diagnostic.json"
...
_VBD_PRECISION_DIAGNOSTIC_V3_EXECUTION_TOKEN = object()
_VBD_PRECISION_DIAGNOSTIC_V3_BOOTSTRAP_CHILD_TOKEN = object()


class VbdTrajectoryPrecisionDiagnosticV3AuthorizationError(RuntimeError):
    """The diagnostic authorization or one-shot claim is invalid."""


def _authorization_error(message: str, exc: Exception | None = None):
    error = VbdTrajectoryPrecisionDiagnosticV3AuthorizationError(message)
    if exc is None:
        raise error
...
def _reject_symlinked_path(path: Path, label: str) -> None:
    ...
    for part in path.parts[1:]:
        ...
        try:
            ...
            _authorization_error(f"{label} cannot be inspected", exc)
        if stat.S_ISLNK(info.st_mode):
            _authorization_error(f"{label} resolves through a symlink")


def _strict_existing_directory(path: Path, label: str) -> tuple[str, ...]:
    _reject_symlinked_path(path, label)
    if not path.exists():
        return ()
    try:
        info = path.lstat()
        if not stat.S_ISDIR(info.st_mode):
            _authorization_error(f"{label} is not a directory")
        return tuple(sorted(os.listdir(path)))
    except OSError as exc:
        _authorization_error(f"{label} cannot be inspected", exc)


def preflight_vbd_precision_diagnostic_v3_fixed_roots(
    *, manifest: dict, claim_consumed: bool
) -> None:
    """Reject unsafe or nonempty fixed roots before generation or sampling."""
    ...
    authorization_path = Path(manifest["execution_authorization_record_path"])
    if _strict_existing_directory(workspace, "diagnostic workspace"):
        _authorization_error("diagnostic workspace is not empty before execution")
    if _strict_existing_directory(checkpoint_root, "diagnostic checkpoint root"):
        _authorization_error(
            "diagnostic checkpoint root is not empty before execution"
        )
    claim_names = _strict_existing_directory(claim_root, "diagnostic claim root")
    expected_claim_names = {authorization_path.name}
    if claim_consumed:
        expected_claim_names.add(
            VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_CLAIM_FILENAME
        )
    if set(claim_names) != expected_claim_names:
        _authorization_error("diagnostic claim root contents are off-plan")
    for name in claim_names:
        path = claim_root / name
        try:
            info = path.lstat()
        except OSError as exc:
            _authorization_error("diagnostic claim-root entry is unavailable", exc)
        if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
            _authorization_error("diagnostic claim-root entry is unsafe")
...
def validate_vbd_precision_diagnostic_v3_authorization_manifest(value: object) -> dict:
    ...
    if (
        ...
        or workspace != Path(VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_WORKSPACE_PATH)
        or claim_root != Path(VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_CLAIM_ROOT_PATH)
        or checkpoint_root
        != Path(VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_CHECKPOINT_ROOT_PATH)
        or any(
            left == right
            or left.is_relative_to(right)
            or right.is_relative_to(left)
            for left in (workspace, claim_root, checkpoint_root)
            for right in (
                ...
                Path(VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V2_CONSUMED_WORKSPACE_PATH),
                Path(VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V2_CONSUMED_CLAIM_ROOT_PATH),
                Path(VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V2_CONSUMED_CHECKPOINT_ROOT_PATH),
            )
        )
        or authorization_path.parent != claim_root
        or output_path.parent != workspace
        or output_path.name != VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_OUTPUT_FILENAME
    ...
    if (
        ...
        != vbd_trajectory_precision_diagnostic_v3_plan()["diagnostic_plan_hash"]
        or value["seed_manifest_hash"]
        != vbd_trajectory_precision_diagnostic_v3_seed_manifest()["seed_manifest_hash"]
...
def build_vbd_precision_diagnostic_v3_authorization_manifest(
    *,
    ...
    output_path = workspace / VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_OUTPUT_FILENAME
    bootstrap = (
        _repo_root() / VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_BOOTSTRAP_RELATIVE_PATH
    ).resolve()
    ...
    return validate_vbd_precision_diagnostic_v3_authorization_manifest(
        {**body, "manifest_hash": sha256_json(body)}
    )


def validate_vbd_precision_diagnostic_v3_execution_authorization(
    value: object,
    *,
    manifest: dict,
    ...
    if (
        ...
        or value["scope"] != VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_AUTHORIZATION_SCOPE
...
def _ensure_private_directory(path: Path) -> int:
    ...
    except (OSError, VbdTrajectoryPrecisionDiagnosticV3AuthorizationError) as exc:
...
def _vbd_precision_diagnostic_v3_claim_body(
    *, manifest: dict, execution_authorization: dict, authorization_commit: str
) -> dict:
    return {
        "schema_version": VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_CLAIM_SCHEMA_VERSION,
...
def validate_vbd_precision_diagnostic_v3_claim(
    value: object,
    *,
    manifest: dict,
    ...
    body = _vbd_precision_diagnostic_v3_claim_body(
        manifest=manifest,
...
def consume_vbd_precision_diagnostic_v3_claim(
    *,
    manifest: dict,
    execution_authorization: dict,
    authorization_commit: str,
    ...
    try:
        _write_exclusive_json_at(
            root_fd=root_fd,
            filename=VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_CLAIM_FILENAME,
            value=claim,
...
def bind_vbd_precision_diagnostic_v3_input(
    *,
    manifest: dict,
    claim: dict,
    ...
    binding = validate_vbd_precision_diagnostic_v3_input_binding(
        binding, claim=claim
    )
    ...
    try:
        _write_exclusive_json_at(
            root_fd=root_fd,
            filename=VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_INPUT_BINDING_FILENAME,
            value=binding,
...
def validate_vbd_precision_diagnostic_v3_input_binding(
    value: object, *, claim: dict
) -> dict:
    expected_keys = {
        "schema_version",
        "claim_hash",
    ...
    if type(value) is not dict or set(value) != expected_keys:
        _authorization_error("diagnostic input binding shape is invalid")
    if (
        type(claim) is not dict
        or value["claim_hash"] != claim.get("claim_hash")
    ):
        _authorization_error("diagnostic input binding claim is invalid")
    for key in (
        "claim_hash",
        ...
        try:
            ...
            _authorization_error(f"{key} is invalid", exc)
    for key in ("prepared_input_hashes", "model_input_hashes"):
        hashes = value[key]
        if type(hashes) is not list or len(hashes) != 3:
            _authorization_error(f"diagnostic {key} are incomplete")
        for item in hashes:
            try:
                _strict_sha256(item, key)
            except Exception as exc:
                _authorization_error(f"diagnostic {key} are invalid", exc)
    body = {key: item for key, item in value.items() if key != "input_binding_hash"}
    if (
        value["schema_version"]
        != "FT_AI_VALUE_VBD_PRECISION_DIAGNOSTIC_INPUT_BINDING_2026_07_V3"
        or value["input_binding_hash"] != sha256_json(body)
        or any(
            item
            in {
                VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V1_CONSUMED_AUTHORIZATION_MANIFEST_HASH,
            ...
            for item in (
                value["claim_hash"],
                value["synthetic_input_hash"],
                value["panel_manifest_root"],
                *value["prepared_input_hashes"],
                *value["model_input_hashes"],
                value["input_binding_hash"],
            )
        )
    ):
        _authorization_error("diagnostic input binding is invalid")
    return value


def write_vbd_precision_diagnostic_v3_staged_output(
    *, manifest: dict, record: dict, checkpoint_root: Path, checkpoint_identity
) -> dict:
    manifest = validate_vbd_precision_diagnostic_v3_authorization_manifest(manifest)
    ...
    try:
        _write_exclusive_json_at(
            ...
            filename=VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_STAGED_OUTPUT_FILENAME,
            value=record,
    ...
    staged_path = (
        workspace / VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_STAGED_OUTPUT_FILENAME
    )
    persisted = read_strict_json(staged_path)
    ...
    if persisted != validated:
        _authorization_error("diagnostic staged semantic readback differs")
    return persisted
...
def validate_vbd_precision_diagnostic_v3_persisted_output(
    *,
    ...
    """Strictly revalidate staged or final persisted bytes without execution."""
    ...
    try:
        binding = validate_vbd_precision_diagnostic_v3_input_binding(
            read_strict_json(binding_path), claim=claim
        )
    except Exception as exc:
        _authorization_error("diagnostic input binding cannot be validated", exc)
    expected_provenance = {
        "authorization_commit": authorization_commit,
        "authorization_manifest_hash": manifest["manifest_hash"],
        "execution_authorization_hash": execution_authorization[
        ...
        "synthetic_input_hash": binding["synthetic_input_hash"],
        "panel_manifest_root": binding["panel_manifest_root"],
    }
    prepared_hashes = [
        lane["prepared_input_hash"] for lane in record["lane_records"]
    ]
    model_hashes = [lane["model_input_hash"] for lane in record["lane_records"]]
    if (
        record["provenance"] != expected_provenance
        or prepared_hashes != binding["prepared_input_hashes"]
        or model_hashes != binding["model_input_hashes"]
    ):
        _authorization_error("diagnostic persisted provenance is off-plan")
    return record


def verify_vbd_precision_diagnostic_v3_authorization_commit(
...
def read_vbd_precision_diagnostic_v3_execution_authorization(
    path: Path,
    *,
    manifest: dict,
    authorization_commit: str,
    ...
    return validate_vbd_precision_diagnostic_v3_execution_authorization(
        value,
        manifest=manifest,
        authorization_commit=authorization_commit,
    )
...
def read_vbd_precision_diagnostic_v3_claim(
    *,
    manifest: dict,
    execution_authorization: dict,
    authorization_commit: str,
    ...
    return validate_vbd_precision_diagnostic_v3_claim(
        claim,
        manifest=manifest,
        execution_authorization=execution_authorization,
        authorization_commit=authorization_commit,
...
def bootstrap_claimed_vbd_precision_diagnostic_v3(
    *,
    manifest: dict,
    ...
    if _bootstrap_token is not _VBD_PRECISION_DIAGNOSTIC_V3_BOOTSTRAP_CHILD_TOKEN:
    ...
    authorization = read_vbd_precision_diagnostic_v3_execution_authorization(
        authorization_path,
        manifest=manifest,
        authorization_commit=authorization_commit,
    )
    claim = read_vbd_precision_diagnostic_v3_claim(
        manifest=manifest,
        execution_authorization=authorization,
        authorization_commit=authorization_commit,
    )
    from .vbd_trajectory_precision_diagnostic_v3_execution import (
        execute_authorized_vbd_precision_diagnostic_v3,
    )
    from .vbd_trajectory_precision_diagnostic_v3_checkpoint import (
        VbdTrajectoryPrecisionDiagnosticV3CheckpointIdentity,
    )

    record = execute_authorized_vbd_precision_diagnostic_v3(
        manifest=manifest,
        execution_authorization=authorization,
        claim=claim,
        _execution_token=_VBD_PRECISION_DIAGNOSTIC_V3_EXECUTION_TOKEN,
    )
    checkpoint_identity = VbdTrajectoryPrecisionDiagnosticV3CheckpointIdentity(
        implementation_commit=manifest["implementation_commit"],
        authorization_commit=authorization_commit,
        authorization_manifest_hash=manifest["manifest_hash"],
        human_execution_authorization_hash=authorization[
            "execution_authorization_hash"
        ],
        attempt_claim_hash=claim["claim_hash"],
    )
    return write_vbd_precision_diagnostic_v3_staged_output(
        manifest=manifest,
        record=record,
        checkpoint_root=Path(manifest["checkpoint_root_path"]),
        checkpoint_identity=checkpoint_identity,
