...
VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_CLAIM_SCHEMA_VERSION = (
    "FT_AI_VALUE_VBD_PRECISION_DIAGNOSTIC_ATTEMPT_CLAIM_2026_07_V1"
)
VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_AUTHORIZATION_SCOPE = (
    "vbd_precision_design_diagnostic_v1_nonacceptance_one_launch"
)
...
_VBD_PRECISION_DIAGNOSTIC_EXECUTION_TOKEN = object()
_VBD_PRECISION_DIAGNOSTIC_BOOTSTRAP_CHILD_TOKEN = object()


class VbdTrajectoryPrecisionDiagnosticAuthorizationError(RuntimeError):
    """The diagnostic authorization or one-shot claim is invalid."""


def _authorization_error(message: str, exc: Exception | None = None):
    error = VbdTrajectoryPrecisionDiagnosticAuthorizationError(message)
    if exc is None:
        raise error
...
def validate_vbd_precision_diagnostic_authorization_manifest(value: object) -> dict:
    ...
    if (
        ...
        or workspace == claim_root
        or workspace.is_relative_to(claim_root)
        or claim_root.is_relative_to(workspace)
        or authorization_path.parent != claim_root
        or output_path.parent != workspace
        or output_path.name != VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_OUTPUT_FILENAME
    ...
    if (
        ...
        != vbd_trajectory_precision_diagnostic_plan()["diagnostic_plan_hash"]
        or value["seed_manifest_hash"]
        != vbd_trajectory_precision_diagnostic_seed_manifest()["seed_manifest_hash"]
...
def build_vbd_precision_diagnostic_authorization_manifest(
    *,
    ...
    output_path = workspace / VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_OUTPUT_FILENAME
    bootstrap = (
        _repo_root() / VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_BOOTSTRAP_RELATIVE_PATH
    ).resolve()
    ...
    return validate_vbd_precision_diagnostic_authorization_manifest(
        {**body, "manifest_hash": sha256_json(body)}
    )


def validate_vbd_precision_diagnostic_execution_authorization(
    value: object,
    *,
    manifest: dict,
    ...
    if (
        ...
        or value["scope"] != VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_AUTHORIZATION_SCOPE
...
def _ensure_private_directory(path: Path) -> int:
    ...
    except (OSError, VbdTrajectoryPrecisionDiagnosticAuthorizationError) as exc:
...
def _vbd_precision_diagnostic_claim_body(
    *, manifest: dict, execution_authorization: dict, authorization_commit: str
) -> dict:
    return {
        "schema_version": VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_CLAIM_SCHEMA_VERSION,
...
def validate_vbd_precision_diagnostic_claim(
    value: object,
    *,
    manifest: dict,
    ...
    manifest = validate_vbd_precision_diagnostic_authorization_manifest(manifest)
    execution_authorization = validate_vbd_precision_diagnostic_execution_authorization(
        execution_authorization,
        manifest=manifest,
        authorization_commit=authorization_commit,
    )
    body = _vbd_precision_diagnostic_claim_body(
        manifest=manifest,
...
def consume_vbd_precision_diagnostic_claim(
    *,
    manifest: dict,
    execution_authorization: dict,
    authorization_commit: str,
    ...
    try:
        _write_exclusive_json_at(
            root_fd=root_fd,
            filename=VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_CLAIM_FILENAME,
            value=claim,
...
def bind_vbd_precision_diagnostic_input(
    *,
    manifest: dict,
    claim: dict,
    ...
    body = {
        "schema_version": "FT_AI_VALUE_VBD_PRECISION_DIAGNOSTIC_INPUT_BINDING_2026_07_V1",
    ...
    try:
        _write_exclusive_json_at(
            root_fd=root_fd,
            filename=VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_INPUT_BINDING_FILENAME,
            value=binding,
...
def verify_vbd_precision_diagnostic_authorization_commit(
...
def read_vbd_precision_diagnostic_execution_authorization(
    path: Path,
    *,
    manifest: dict,
    authorization_commit: str,
    ...
    return validate_vbd_precision_diagnostic_execution_authorization(
        value,
        manifest=manifest,
        authorization_commit=authorization_commit,
    )
...
def read_vbd_precision_diagnostic_claim(
    *,
    manifest: dict,
    execution_authorization: dict,
    authorization_commit: str,
    ...
    claim_path = (
        Path(manifest["external_claim_root_path"])
        / VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_CLAIM_FILENAME
    )
    try:
        claim = read_strict_json(claim_path)
    except Exception as exc:
        _authorization_error("diagnostic launch claim cannot be read", exc)
    return validate_vbd_precision_diagnostic_claim(
        claim,
        manifest=manifest,
        execution_authorization=execution_authorization,
        authorization_commit=authorization_commit,
...
def bootstrap_claimed_vbd_precision_diagnostic(
    *,
    manifest: dict,
    ...
    if _bootstrap_token is not _VBD_PRECISION_DIAGNOSTIC_BOOTSTRAP_CHILD_TOKEN:
    ...
    authorization = read_vbd_precision_diagnostic_execution_authorization(
        authorization_path,
        manifest=manifest,
        authorization_commit=authorization_commit,
    )
    claim = read_vbd_precision_diagnostic_claim(
        manifest=manifest,
        execution_authorization=authorization,
        authorization_commit=authorization_commit,
    )
    from .vbd_trajectory_precision_diagnostic_execution import (
        execute_authorized_vbd_precision_diagnostic,
    )

    record = execute_authorized_vbd_precision_diagnostic(
        manifest=manifest,
        execution_authorization=authorization,
        claim=claim,
