"""Create-once postmortem checkpoints for the non-evidentiary V3 diagnostic."""
...
class VbdTrajectoryPrecisionDiagnosticV3CheckpointIdentity:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or self.authorization_commit
            == VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V2_CONSUMED_AUTHORIZATION_COMMIT
            or self.authorization_manifest_hash
            == VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V2_CONSUMED_AUTHORIZATION_MANIFEST_HASH
            or self.human_execution_authorization_hash
            == VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V2_CONSUMED_EXECUTION_AUTHORIZATION_HASH
            or self.attempt_claim_hash
...
def start_vbd_precision_diagnostic_v3_checkpoint_session(
    *,
    root: Path,
    identity: VbdTrajectoryPrecisionDiagnosticV3CheckpointIdentity,
) -> _CheckpointSession:
    ...
    if type(identity) is not VbdTrajectoryPrecisionDiagnosticV3CheckpointIdentity:
        raise VbdTrajectoryPrecisionDiagnosticV3CheckpointError(
...
def write_vbd_precision_diagnostic_v3_checkpoint(
    ...
    *,
    ...
    if (
        ...
        < len(VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_CHECKPOINT_SEQUENCE)
    ):
        raise VbdTrajectoryPrecisionDiagnosticV3CheckpointError(
    ...
    body = {
        "schema": VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_CHECKPOINT_SCHEMA,
        "diagnostic_identity": VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_V3_ID,
