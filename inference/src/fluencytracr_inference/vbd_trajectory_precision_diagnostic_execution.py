"""Private one-shot execution for the sanitized VBD MCSE diagnostic."""
...
from .vbd_trajectory_nuts import (
    ...
    _sample_vbd_trajectory_precision_diagnostic,
...
from .vbd_trajectory_precision_diagnostic_authorization import (
    _VBD_PRECISION_DIAGNOSTIC_EXECUTION_TOKEN,
    bind_vbd_precision_diagnostic_input,
...
from .vbd_trajectory_synthetic import (
    _PRECISION_DIAGNOSTIC_GENERATION_RUNNER_TOKEN,
    generate_vbd_trajectory_precision_diagnostic_case,
)
...
def execute_authorized_vbd_precision_diagnostic(
    *,
    manifest: dict,
    ...
    """Execute once after the external claim has already been consumed."""

    if _execution_token is not _VBD_PRECISION_DIAGNOSTIC_EXECUTION_TOKEN:
        raise PermissionError("precision diagnostic execution requires its token")
    ...
    try:
        claim = validate_vbd_precision_diagnostic_claim(
            claim,
            manifest=manifest,
        ...
        raise PermissionError("precision diagnostic claim is invalid") from exc
    ...
    if (
        ...
        raise RuntimeError("precision diagnostic runtime or model binding drifted")

    case = generate_vbd_trajectory_precision_diagnostic_case(
        _runner_token=_PRECISION_DIAGNOSTIC_GENERATION_RUNNER_TOKEN
    )
    prepared = tuple(
    ...
    input_binding = bind_vbd_precision_diagnostic_input(
        manifest=manifest,
        claim=claim,
    ...
    for lane_ordinal, lane in enumerate(VBD_TRAJECTORY_LANES):
        ...
        idata = _sample_vbd_trajectory_precision_diagnostic(
            prepared[lane_ordinal],
            case.panel,
            binding=binding,
            _runner_token=_VBD_TRAJECTORY_PRECISION_DIAGNOSTIC_SAMPLING_TOKEN,
        )
        try:
            lane_records.append(
                project_vbd_trajectory_precision_diagnostic_lane(
                    ...
                    lane_ordinal=lane_ordinal,
                    binding=binding,
                    prepared_input_hash=prepared[lane_ordinal].prepared_input_hash,
    ...
    return validate_vbd_trajectory_precision_diagnostic_record(
        build_vbd_trajectory_precision_diagnostic_record(
