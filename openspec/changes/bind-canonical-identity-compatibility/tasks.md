...
- [ ] 1.1 Add a server-owned creation-attestation envelope to new E-capable
...
- [ ] 1.2 Add optional version-bearing metric-definition refs and a separately
...
- [ ] 1.3 Add strict canonical identity core, final binding, selector,
...
- [ ] 1.4 Add fail-first shared/backend tests for missing versions, latest-only
...
- [ ] 1.5 Run `npx openspec validate bind-canonical-identity-compatibility
      --strict`.

## 2. Exact source and compatibility authority

- [ ] 2.1 Add exact-version hypothesis, plan, and Measurement Cell loaders plus
      strict semantic hashes and same-org, no-fork, no-gap, non-superseded
      validation in the minimal-persistence repository.
- [ ] 2.2 HMAC-attest new E-capable hypothesis versions and stamp source-owned
...
- [ ] 2.3 Exact-compare hypothesis, plan, cell, metric ref/hash, source, unit,
...
- [ ] 2.4 Prove an invalid supplied selector never falls back to the unbound
      compatibility path.
- [ ] 2.5 Add the same deterministic transaction advisory lock for every
...
- [ ] 2.6 Add one migration-backed, internal
...
- [ ] 3.1 Add the optional canonical core commitment to D claim, packet,
      ...
      and binding IDs.
- [ ] 3.2 Add the fourth reserved internal binding object, deterministic
      packet-derived ID, generic-API isolation, and insert-or-exact semantics.
- [ ] 3.3 Add a Slice-E-only service-held HMAC creation attestation to the
...
- [ ] 3.4 Extend the serializable D transaction to take source-family advisory
...
- [ ] 3.5 Define a fixed deterministic readout renderer/projection version,
...
- [ ] 3.6 Revalidate the entire chain, creation attestation, renderer version,
...
- [ ] 3.7 Keep raw source selectors, attestation secrets, and row locators out
      of responses,
      rendered HTML, generic APIs, and the public-safe binding payload.

## 4. PostgreSQL and repository verification

- [ ] 4.1 Add focused real-PostgreSQL probes for exact success/replay,
...
- [ ] 4.2 Run focused shared/backend identity, D authorization, value-chain,
...
- [ ] 4.5 Update only the Slice E queue status/last note and canonical
