...
- [x] 1.1 Add the confidence-engine observation requirement statement fixture
...
- [x] 1.2 Add the confidence-engine series read-path decision runner emitting
...
- [x] 1.3 Extend `run_ai_value_durable_series_read_path_decision.mjs` so the
...
- [x] 1.4 Extend `run_ai_value_measurement_cell_series_persistence_promotion_gate.mjs`
      to accept the confidence read-path proof as an alternative READY path.

## 2. Boundary enforcement

- [x] 2.1 Keep every existing FALSE feed false except `research_model_feed`,
      which becomes the scoped token `internal_confidence_engine_only`.
- [x] 2.2 Add validation tests proving any customer-facing, finance, live
      connector, export, route, or UI feed flips the decision to
      `REJECTED_FOR_BOUNDARY_LEAKAGE`.
- [x] 2.3 Add validation tests proving milestone days other than
      [0, 30, 60, 90, 180, 365], missing admission reason codes, or non-compact
      refs are rejected.

## 3. Governance and hygiene

- [x] 3.1 Record human approval (decision owner: James Kelley) on this
      proposal before starting section 1.
- [x] 3.2 Register the new runner/tests without adding bespoke npm scripts if
      a parameterized runner exists by implementation time; otherwise follow
      the current `run:`/`test:` naming convention.
- [x] 3.3 Update the README capability ledger and `docs/contracts/` alongside
      the decision artifacts, per the PR invariants checklist.
- [x] 3.4 Run `openspec validate add-ai-value-series-confidence-read-path --strict`
