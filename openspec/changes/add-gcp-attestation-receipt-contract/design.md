...
### External full-Section-7.5 approval interface

Section 7.4 defines a closed, nonauthorizing interface for externally authenticated full-Section-7.5 approval/policy/verifier records. The only admissible target is canonical bytes for `FULL_SECTION_7_5`, with its exact SHA-256 and target-binding commitment; every `SECTION_7_5A` kind, schema, domain, or hash substitution rejects before an acceptance node is considered. The interface requires the exact approved current head and a typed lineage evidence record covering authenticated current head, strict monotonic predecessors, shared linearizable check-and-use, independent nonrollbackable external anchoring, stale-reader rejection, whole-state-restore detection, and fail-closed recovery before and after commit.

Each of the nine Section 7.5 acceptance nodes—trust distribution, channel enforcement, pre/terminal quote transport, KMS transport, audit mapping, and initial/current/final replay retention—requires the conjunction of that exact target binding, one externally authenticated approval/policy/verifier record, one trust-lineage evidence record, and node-specific evidence. P14 requires this approval and lineage conjunction specifically for `trust_distribution_acceptance_hash`. P19 is limited here to the parent approval interface; choosing an anchor or defining writer, reader, currentness, concurrency, recovery, storage, signature/key, or lifecycle mechanics remains exclusively full Section 7.5 work. All live approval registries remain empty and `authority_effect` remains `NONE`.
