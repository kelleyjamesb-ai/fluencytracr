...
// under OpenSpec change add-confidence-engine-workspace (task 3.1, module 12).
...
// ES2020 TypeScript lib target (identical behavior).
...
function reviewedFitArtifactRef(sourceRuntime: any, ready: boolean): AnyRecord {
...
function buildReviewChecks(sourceRuntime: any, ready: boolean): AnyRecord {
  const artifact = asRecord(sourceRuntime?.internal_fit_artifact);
  return {
...
function buildReview(sourceRuntime: any, state: string, gaps: string[]): AnyRecord {
...
export function contributionAlignmentPosteriorOutputReviewGateHash(review: unknown): string {
  const withoutHash = clone(review);
  delete withoutHash.review_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentPosteriorOutputReviewGateFromObject(input: unknown): AnyRecord {
...
export function validateContributionAlignmentPosteriorOutputReviewGate(review: any, options: AnyRecord = {}): AnyRecord {
