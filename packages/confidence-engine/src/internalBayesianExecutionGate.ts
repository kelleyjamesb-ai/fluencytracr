...
// under OpenSpec change add-confidence-engine-workspace (task 3.1, module 7).
...
function sourceSpecificationFromInput(input: unknown): any {
  const record = asRecord(input);
  return record.source_specification ?? input;
}

function sourceReadinessReviewFromInput(input: unknown, options: AnyRecord = {}): any {
  const record = asRecord(input);
...
function sourceSpecificationRef(sourceSpecification: any): AnyRecord {
  return {
    schema_version:
...
function sourceReadinessReviewRef(sourceSpecification: any, sourceReadinessReview: any): AnyRecord {
...
function sourceFrameRef(sourceSpecification: any, sourceFrame: any): AnyRecord {
...
function sourceSpecificationGaps(sourceSpecification: any): string[] {
...
function sourceReadinessReviewGaps(sourceReadinessReview: any, sourceSpecification: any): string[] {
...
function sourceFrameGaps(sourceFrame: any, sourceSpecification: any): string[] {
...
function buildGatedFeatureWeights(sourceSpecification: any): AnyRecord[] {
...
function buildRuntimePrerequisites(ready: boolean): Record<string, boolean> {
  return Object.fromEntries(RUNTIME_PREREQUISITE_FIELDS.map((field) => [field, ready]));
}

function buildGate(
  sourceSpecification: any,
  sourceReadinessReview: any,
  sourceFrame: any,
  state: string,
  gaps: string[]
): AnyRecord {
...
export function contributionAlignmentInternalBayesianExecutionGateHash(gate: unknown): string {
  const withoutHash = clone(gate);
  delete withoutHash.gate_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentInternalBayesianExecutionGateFromObject(
  input: unknown,
  options: AnyRecord = {}
): AnyRecord {
...
function collectSourceBindingGaps(gate: unknown, options: AnyRecord = {}): string[] {
  const gaps: string[] = [];
...
export function validateContributionAlignmentInternalBayesianExecutionGate(gate: any, options: AnyRecord = {}): AnyRecord {
