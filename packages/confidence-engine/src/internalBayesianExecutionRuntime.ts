...
// under OpenSpec change add-confidence-engine-workspace (task 3.1, module 8).
// Byte-compatibility contract enforced by the golden parity suite. This is the
// fixture-only execution prototype: closed-form Normal-Normal update over a
// four-window aggregate DiD design, held for output review; all posterior,
// confidence, probability, and customer outputs remain blocked.
...
function sourceGateFromInput(input: unknown): any {
  const record = asRecord(input);
  return record.source_gate ?? input;
}

function aggregateWindowsFromInput(input: unknown): any[] {
  const record = asRecord(input);
...
function sourceGateRef(sourceGate: any): AnyRecord {
  return {
    schema_version:
...
function hasForbiddenContent(value: unknown, path = "runtime"): string[] {
...
function sourceGateGaps(sourceGate: any): string[] {
...
function aggregateWindowGaps(windows: unknown): string[] {
  const gaps: string[] = [];
...
function findWindow(windows: unknown, comparisonRole: string, windowRole: string): any {
...
function buildFitArtifact(windows: unknown): AnyRecord {
...
function buildDesignMatrix(windows: unknown): AnyRecord {
...
function buildModelEquation(): AnyRecord {
  return {
...
function buildRuntime(sourceGate: any, windows: any[], state: string, gaps: string[]): AnyRecord {
...
function rejectedRuntime(): AnyRecord {
  const runtime: AnyRecord = {
    schema_version: CONTRIBUTION_ALIGNMENT_INTERNAL_BAYESIAN_EXECUTION_RUNTIME_SCHEMA_VERSION,
    runtime_id: null,
...
export function contributionAlignmentInternalBayesianExecutionRuntimeHash(runtime: unknown): string {
  const withoutHash = clone(runtime);
  delete withoutHash.runtime_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentInternalBayesianExecutionRuntimeFromObject(input: unknown): AnyRecord {
...
function rejectValidation(runtime: any): AnyRecord | null {
...
function collectShapeGaps(runtime: any): string[] {
...
function collectSourceBindingGaps(runtime: any, options: AnyRecord = {}): string[] {
  const gaps: string[] = [];
...
export function validateContributionAlignmentInternalBayesianExecutionRuntime(runtime: any, options: AnyRecord = {}): AnyRecord {
  if (runtime?.runtime_state === REJECT_STATE) {
    return rejectValidation(runtime) as AnyRecord;
  }
  const gaps = sanitizeGaps([
