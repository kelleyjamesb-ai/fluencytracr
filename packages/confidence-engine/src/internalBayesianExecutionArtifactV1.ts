...
// under OpenSpec change add-confidence-engine-workspace (task 3.1, module 15).
...
function sourceHandoffFromInput(input: any): any {
  const record = asRecord(input);
...
function buildSources(input: any): AnyRecord {
  const record = asRecord(input);
...
function promotionHandoffRef(handoff: any, includeSourceDetails = true): AnyRecord {
  return {
...
function promotionGateRef(handoff: any, sourceGate: any, includeSourceDetails = true): AnyRecord {
...
function sourceHashes(handoff: any): AnyRecord {
  return asRecord(handoff?.source_hashes);
}

function sourceRuntimeRef(handoff: any, sourceRuntime: any): AnyRecord {
...
function sourceDiagnosticsReviewRef(handoff: any, sourceReview: any): AnyRecord {
  const hashes = sourceHashes(handoff);
  return {
...
function sourceDiagnosticsEvidencePacketRef(handoff: any, sourcePacket: any): AnyRecord {
  const hashes = sourceHashes(handoff);
  return {
...
function sourceGovernedDiagnosticsSufficiencyEvidenceSourceRef(handoff: any, sourceGovernedSource: any): AnyRecord {
  const hashes = sourceHashes(handoff);
  return {
...
function modelExecutionScope(): AnyRecord {
  return {
    internal_only: true,
    aggregate_only: true,
...
function diagnosticEvidenceBinding(sources: AnyRecord): AnyRecord {
...
function posteriorValuesContainmentPolicy(): AnyRecord {
  return {
...
function interpretationPolicy(): AnyRecord {
  return {
    posterior_interpretation_authorized: false,
...
function artifactPolicy(): AnyRecord {
  return {
    internal_only: true,
    aggregate_only: true,
...
function sourceGaps(sources: AnyRecord): string[] {
  ...
  if (handoffValidation.valid !== true) {
    ...
    gaps.push(...handoffValidation.gaps.map((gap: string) => `promotion_handoff.${gap}`));
  }
...
function buildArtifact(sources: AnyRecord, state: string, gaps: string[]): AnyRecord {
...
function rejectedArtifact(): AnyRecord {
  const artifact: AnyRecord = {
    schema_version:
      CONTRIBUTION_ALIGNMENT_INTERNAL_BAYESIAN_EXECUTION_ARTIFACT_V1_SCHEMA_VERSION,
    artifact_id: null,
...
export function contributionAlignmentInternalBayesianExecutionArtifactV1Hash(artifact: unknown): string {
  const withoutHash = clone(artifact);
  delete withoutHash.artifact_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentInternalBayesianExecutionArtifactV1FromObject(input: unknown): AnyRecord {
...
function validateSourceRefs(artifact: any, options: AnyRecord): string[] {
  const gaps = [];
