...
// under OpenSpec change add-confidence-engine-workspace (task 3.1, module 10).
...
function evidenceHash(label: string, value: unknown): string {
...
function sufficiencyDimension(evidence: any, dimension: string): any {
  return asRecord(asRecord(evidence).evidence_dimensions)[dimension] ?? null;
}

function buildDataAdequacyEvidence(sourceRuntime: any, ready: boolean): AnyRecord {
...
function buildWindowReview(sourceRuntime: any, ready: boolean): AnyRecord {
  const design = asRecord(sourceRuntime?.aggregate_design_matrix);
  const evidence: AnyRecord = {
...
function buildModelDiagnosticsEvidence(
  sourceRuntime: any,
  sourceEvidence: any,
  ready: boolean,
  evidenceReady: boolean
...
function buildComparisonDesignEvidence(
  sourceRuntime: any,
  sourceEvidence: any,
  ready: boolean,
  evidenceReady: boolean
...
function buildFeatureWeightProvenance(sourceRuntime: any, ready: boolean): AnyRecord {
  const evidence: AnyRecord = {
...
function buildEvidenceSufficiency(packet: AnyRecord): AnyRecord {
  return {
    data_adequacy_satisfied:
...
function addAllEvidenceSatisfied(evidence: AnyRecord): AnyRecord {
  return {
    ...evidence,
    all_required_evidence_satisfied:
...
function rejectedPacket(): AnyRecord {
  const packet: AnyRecord = {
    schema_version: CONTRIBUTION_ALIGNMENT_DIAGNOSTICS_EVIDENCE_PACKET_SCHEMA_VERSION,
    packet_id: null,
...
export function contributionAlignmentDiagnosticsEvidencePacketHash(packet: unknown): string {
  const withoutHash = clone(packet);
  delete withoutHash.packet_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentDiagnosticsEvidencePacketFromObject(input: unknown): AnyRecord {
...
function hasForbiddenContent(value: unknown, path = "packet"): string[] {
...
function validatePacketShape(packet: any): string[] {
...
export function validateContributionAlignmentDiagnosticsEvidencePacket(packet: any, options: AnyRecord = {}): AnyRecord {
