...
// under OpenSpec change add-confidence-engine-workspace (task 3.1, module 2).
...
function sourceReviewFromInput(input: unknown): any {
...
function sourceRef(sourceReview: any): AnyRecord {
  return {
    schema_version:
...
function hasForbiddenContent(value: unknown, path = "decision"): string[] {
...
function sourceReviewGaps(sourceReview: any): string[] {
...
function buildDecision(sourceReview: any, state: string, gaps: string[]): AnyRecord {
  const ready = state === READY_STATE;
  const decision: AnyRecord = {
...
function rejectedDecision(gaps: string[]): AnyRecord {
  return buildDecision(null, REJECT_STATE, gaps);
}

export function contributionAlignmentInternalNumericWeightDecisionHash(decision: unknown): string {
  const withoutHash = clone(decision);
  delete withoutHash.decision_hash;
  return sha256Json(withoutHash);
}

export function buildContributionAlignmentInternalNumericWeightDecisionFromObject(input: unknown): AnyRecord {
...
function collectShapeGaps(decision: unknown): string[] {
...
function collectSourceBindingGaps(decision: unknown, options: AnyRecord = {}): string[] {
...
export function validateContributionAlignmentInternalNumericWeightDecision(decision: unknown, options: AnyRecord = {}): AnyRecord {
  ...
  return {
    ...
    valid: gaps.length === 0 && (decision as AnyRecord)?.decision_state === READY_STATE,
    gaps
  };
}
