...
// under OpenSpec change add-confidence-engine-workspace (task 3.1, module 9).
// Byte-compatibility contract enforced by the golden parity suite. Includes
// the merge-collision resolution recorded in PORTING.md: envelope gate/window
// options bind when both are present, else self-contained runtime validation.
...
function runtimeEnvelopeSidecarContentGaps(value: unknown, path = "source_runtime_envelope"): string[] {
...
function governedReviewedSourceEvidenceHash(dimension: string): string {
  return sha256Json({
...
function dimensionHashFromBoundHashes(
  ...
  sourceFixtureArtifactHash: string | null,
  dimension: string,
...
function reviewedEvidenceManifestHash(manifest: unknown): string {
...
function sourceRuntimeGaps(sourceRuntime: any, validationOptions: AnyRecord = {}): string[] {
  ...
  if (validation.valid !== true) {
    ...
    gaps.push(...validation.gaps.map((gap: string) => `source_runtime.${gap}`));
  }
...
function reviewedEvidenceContentGaps(value: unknown, path = "reviewed_diagnostics_source_evidence"): string[] {
  if (!value) return [];
  if (Array.isArray(value)) {
...
function reviewedEvidenceEnvelopeGaps(sourceRuntime: any, reviewedEvidence: any): string[] {
...
function reviewedEvidenceManifestGaps(sourceRuntime: any, reviewedEvidence: any): string[] {
  const evidence = asRecord(reviewedEvidence);
...
function dimensionGaps(sourceRuntime: any, reviewedEvidence: any, dimension: string): string[] {
...
function dimensionMissingEvidenceRequirements(sourceRuntime: any, reviewedEvidence: any, dimension: string): string[] {
  const detail = asRecord(asRecord(asRecord(reviewedEvidence).evidence_dimensions)[dimension]);
  const manifestDetail = asRecord(asRecord(
    asRecord(asRecord(reviewedEvidence).reviewed_evidence_manifest).evidence_dimensions
  )[dimension]);
...
function reviewedEvidenceGaps(sourceRuntime: any, reviewedEvidence: any): string[] {
  if (!reviewedEvidence) return ["reviewed diagnostics source evidence is required"];
  return sanitizeGaps([
...
function allEvidenceReady(sourceRuntime: any, reviewedEvidence: any): boolean {
  return reviewedEvidenceGaps(sourceRuntime, reviewedEvidence).length === 0;
}

function buildDimension(sourceRuntime: any, reviewedEvidence: any, dimension: string, ready: boolean): AnyRecord {
...
function buildDimensions(sourceRuntime: any, reviewedEvidence: any, ready: boolean): AnyRecord {
...
function buildFeatureWeightProvenance(dimensions: AnyRecord, ready: boolean): AnyRecord {
  return {
...
function buildEvidenceSufficiency(dimensions: AnyRecord): AnyRecord {
  return {
    comparison_design_adequacy_satisfied:
...
function buildEvidenceReadinessReconciliation(
  sourceRuntime: any,
  reviewedEvidence: any,
  sourceGaps: string[],
  evidenceGaps: string[]
): AnyRecord {
...
function buildSource(
  ...
  state: string,
  gaps: string[],
  sourceGapsOverride?: string[],
  evidenceGapsOverride?: string[]
): AnyRecord {
...
function rejectedSource(): AnyRecord {
  const source: AnyRecord = {
    schema_version:
      CONTRIBUTION_ALIGNMENT_GOVERNED_DIAGNOSTICS_SUFFICIENCY_EVIDENCE_SOURCE_SCHEMA_VERSION,
    source_id: null,
...
export function contributionAlignmentGovernedDiagnosticsSufficiencyEvidenceSourceHash(source: unknown): string {
...
function packetSideEvidenceHash(evidence: unknown): string {
  const withoutHash = clone(evidence);
  delete withoutHash.evidence_hash;
  return sha256Json(withoutHash);
}

function packetProjectionGaps(source: unknown): string[] {
...
export function buildContributionAlignmentDiagnosticsSufficiencyEvidenceFromGovernedSource(source: unknown): AnyRecord | null {
  const record = asRecord(source);
...
export function buildContributionAlignmentGovernedDiagnosticsSufficiencyEvidenceSourceFromObject(input: unknown): AnyRecord {
...
function hasForbiddenContent(value: unknown, path = "source"): string[] {
  if (Array.isArray(value)) {
    if (
...
function validateShape(source: any): string[] {
...
function validateAgainstSources(source: any, options: AnyRecord = {}): string[] {
  const gaps: string[] = [];
...
export function validateContributionAlignmentGovernedDiagnosticsSufficiencyEvidenceSource(source: any, options: AnyRecord = {}): AnyRecord {
