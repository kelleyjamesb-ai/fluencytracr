...
2. The first-class `value_evidence_case` contract now exists
   (`schemas/ai-value-intelligence/value-evidence-case.schema.json`,
   `shared/src/aiValueEngine/valueEvidenceCase.ts`); route further UI expansion
   through it instead of adding parallel evidence surfaces.
