...
Staggered rollout may be collected as comparison-design context only. It must
not route to the current `comparison_supported_bayesian_did_module`; it HOLDS
until a future approved model implements and calibrates true event-time,
calendar-time, and not-yet-treated logic.
