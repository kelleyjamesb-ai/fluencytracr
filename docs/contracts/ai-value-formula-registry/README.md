...
| `first_longitudinal_synthetic_model_slice` | `inference/src/fluencytracr_inference/longitudinal_state_space.py` | `IMPLEMENTED_SYNTHETIC_ONLY` | State-space/NUTS-concordant and replicated-validated synthetic associational VBD-outcome contrast | Keep synthetic/internal only; no real-data or customer output authority |
...
| `evidence_design_claim_cap` | Bayesian model-family contract | `SPECIFIED_NOT_IMPLEMENTED` | Claim-strength cap prebound from approved design and enforced after estimation | Keep specified; later review may only lower or HOLD; no runtime upgrade |
